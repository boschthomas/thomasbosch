...
According to the expressivity of constraint languages, the complete set of \emph{constraint types} encompasses three disjoint \emph{sets of constraint types}:
\begin{enumerate}
...
We further distinguished \emph{Constraint Language Based} as the set of constraint types that are not \emph{RDF/OWL Based}, but can be expressed by common declarative high-level constraint languages like ShEx, ReSh, and DSP. SPARQL is considered as a low-level implementation language in this context, not a high-level constraint language. 
...
\subsubsection*{RDFS/OWL Based Constraint Types}
...
\subsubsection*{Constraint Language Based}
...
\subsubsection*{SPARQL Based Constraint Types}
\label{SPARQL-based-constraint-types}
...
In general, \emph{vocabulary constraints} are expressible by either modeling languages (e.g., RDF, RDFS, \emph{OWL 2}) or constraint languages. 
\emph{Simple constraints} and especially \emph{complex constraints}, however, are in most cases only expressible by plain SPARQL. 
