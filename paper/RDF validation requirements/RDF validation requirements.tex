...
\begin{itemize}
  \item Constraints of 64\% of all constraint type can be expressed using logical constructs from description logics.
	\item We developed a vocabulary and for each constraint type we identified constraining elements to represent constraints of any constraint type. 
	
	\item In most cases, it is sufficient to use only 1 logical construct
	\item Some combinations of logical constructs can be simplified to 1 logical construct which can be used in terms of syntactic sugar
  \item constraints of the majority of the constraint types can be expressed logically by description logics
	\item constraints of all constraint types can be expressed by constraining elements
	\item For 1 constraint types, there can be multiple constraining elements
...
*Property Domain & $\exists$, $\sqsubseteq$ & domain \\
*Property Range & $\sqsubseteq$, $\forall$ & range \\
...
Context-Sp. Property Groups & $\sqcap$ & property groups \\
...
*Transitive Object Properties & $\sqsubseteq$ & transitive \\
Context-Sp. Exclusive OR of P. & $\neg$, $\sqcap$, $\sqcup$ & XOR \\
Context-Sp. Exclusive OR of P. Groups & $\neg$, $\sqcap$, $\sqcup$ & XOR, $\sqcap$ \\
Allowed Values & $\sqcup$ & allowed \\
...
Negative Literal Ranges & - & xsd:minInclusive \\
                        & & xsd:maxInclusive \\
Required Properties & $\exists$ & required \\	
...
Optional Properties & $\exists$, $\sqsubseteq$ & optional \\
Repeatable Properties & $\geq$ & repeatable \\
Negative Property Constraints & $\neg$, $\exists$ & negative properties \\
*Individual Equality & = & \\
*Functional Properties & functional & \\ 
*Inverse-Functional Properties & inverse, functional & inverse functional \\
...
HTML Handling & - & html \\
Conditional Properties & $\sqsubseteq$ & conditional \\
...
Use Sub-Super Relations in Validation & - & not redundant \\
*Cardinality Shortcuts & & optional \& non-repeatable, \\
                       & & optional \& repeatable \\
