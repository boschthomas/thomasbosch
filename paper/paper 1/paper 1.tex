...
\begin{itemize}
	...
	\item The purpose of the paper is to provide a basis for the development of constraint languages.
...
BTW, it is completely misleading to say in introduction  "Reasoning is a promising solution as a pre-validation step to overcome this shortcoming": reasoning must be part of RDF data management as soon as RDFS statements are declared (otherwise, why would they be declared ?).
	...
	\end{itemize}
	\item If reasoning can be performed or if it does not make any sense to use it depends on the constraint type to be expressed and not on the constraint language. Although, reasoning is not yet implemented for other constraint languages than OWL 2, these constraint languages can easily be extended.
...
-----
	...
	\begin{itemize}
		\item The intention of the paper is not to present any concrete normative suggestion or solution on how constraints for RDF validation should be represented.
		The intention of this paper is not to suggest how RDF constraints should be represented as there are already five promising constraint languages which can be used to formulate RDF constraints. The developed constraint languages’ expressivity scores, however, enable systems to recommend constraint languages depending on individual use cases and therefore needed constraint types. 
	\end{itemize}
...
\begin{enumerate}
	...
	\item When combining reasoning and validation, the distinction between the CWA with UNA (CWA/UNA ) or the OWA without nUNA (OWA/nUNA)  is an important aspect. Thus, we evaluate each constraint type regarding the CWA/UNA dependency.\tb{ToDo}
...
\subsection{Dependency on CWA and Dependency on UNA}
...
The distinction of constraint types regarding the dependency on the CWA and on the UNA is an important aspect when combining reasoning with validation.
As many OWL 2 axioms correspond to constraint types we want to use for validation, 
we have to know their behavior in terms of validation results in different semantic settings. 
OWL 2 reasoning requires the OWA with the nUNA  whereas RDF validation has its origin in the XML world and is therefore classically based on the CWA and the UNA.
When assuming OWA and nUNA for RDF validation, validation won’t be that restrictive and we won’t get the intended validation results.
While DLs/OWL do not assume UNA, they have constructs to express when two names are the same or not.
...
We classify constraint types according to the dependency on the CWA and the UNA which leads to not disjoint sets of constraint types:
...
{\em Minimum qualified cardinality restrictions} (\emph{R-75}) is a $\mathcal{CT}_{CWA}$ constraint type, i.e., depends on the CWA.
...
Nothing can be a \emph{book} and a \emph{journal article}  at the same time (\ms{Book $\sqcap$ JournalArticle $\sqsubseteq$ $\perp$}).
For {\em disjoint classes} (\emph{R-7}), it does not make any difference if the CWA or the OWA is taken,
as if there is a publication which is a book and a journal article a constraint violation is raised in both settings.
%as all information needed for validation is explicitly available in both cases. 
...
For \emph{Functional properties} (\emph{R-57/65}), it makes a difference if UNA or nUNA is assumed.
...
%(\ms{title(Huckleberry-Finn, The-Adventures-of-Huckleberry-Finn)}, \ms{title(Huckleberry-Finn, Die-Abenteuer-des-Huckleberry-Finn)}).
For nUNA, however, reasoning concludes that the title {\em The-Adventures-of-Huckleberry-Finn} is the same as the title {\em Die-Abenteuer-des-Huckleberry-Finn} 
...
(4) $\mathcal{CT}_{nUNA}$ (complement of $\mathcal{CT}_{UNA}$) denotes the set of constraint types which are dependent on the \emph{nUNA}.

-----

For this work, following the similar fashion, we evaluated for each constraint type 
if it is dependent or independent on the CWA and on the UNA (for a detailed analysis see \cite{BoschNolleAcarEckert2015}). 
The result of this evaluation is that we distinguish between 42 $\mathcal{C}_{CWA}$ and 32 $\overline{\mathcal{C}_{CWA}}$ constraint types.
Hence, for the majority of constraint types it makes a difference if the CWA or the OWA is assumed and if the UNA or the nUNA is assumed.
\tb{ToDo: evaluation}
...
\subsection{About the Complexity}
