...
%
\title{RDF Validation}
\subtitle{Effects on Complexity with and without Reasoning}
%
...
The database on RDF validation requirements is intended to be a way to collect case studies, use cases and especially requirements regarding RDF validation collaboratively and in a structured form.
I initiated this comprehensive database obtained from various sources, including the use cases presented at the W3C RDF validation workshop as well as discussions in the W3C and DCMI WGs. 
The database, which is openly available at \url{http://purl.org/net/rdf-validation}, is used to evaluate and to compare several existing solutions for RDF constraint formulation and validation. 
We classify the requirements to provide a high-level view on different solutions and to facilitate a better understanding of the problem domain \cite{BoschEckert2014}.  
Members of the DCMI RDF-AP WG published a deliverable on RDF validation case studies, use cases, and requirements collected within the Dublin Core community
\cite{DroegeBoschCharlesClayphanMatienzoRuehlePohlAlonenSvenssonCoyle2014}.
The RDF validation database with its case studies, use cases, requirements, and solutions is continuously maintained and extended by the members of the W3C RDF Data Shapes WG and the DCMI RDF-AP WG. 

There are multiple domain-specific constraint languages (DSCLs) (the most important ones are OWL 2, DSP, ShEx, and ReSh) having different syntaxes and semantics which can be used to express constraints (like at-least restrictions), 
but there is no single standard DSCL which can be used to express any constraint for each purpose.
...
\textbf{ToDo Thomas: Automatic Validation of RDF Constraints Expressed by any DSCL}
...
\tb{I'm not aware of this, but if you think it is worth - go ahead!}
...
\textbf{Literal Pattern Matching.} \an{IMHO literal pattern matching is not expressible in DL!}\tb{but we can express it using OWL 2 DL as it is shown in the OWL 2 spec}
...
personDescriptionTemplate
    ...
    dsp:statementTemplate [
        ...
        dsp:literalConstraint [
            ...
            dsp:literal "white"] ] .
\end{ex}
\an{One constraint language is IMHO enought... Maybe not!?}
...
\end{DL} \an{IMHO: If you use the universal quantifier it is also valid that a Jedi neither have attackingBySword nor attackingByForce, so it should be an existential quantifier...}
...
The same constraint may be expressed by OWL 2 DL \an{Do you mean DL or QL at this point?}, but not very intuitively and concisely.
...
:CC2 rdfs:subClassOf [
    ...
    owl:allValuesFrom xsd:boolean ] .
\end{ex}
\an{Formulars in DL would be nice... ;)}
...
\an{coloring still to be checked!}
