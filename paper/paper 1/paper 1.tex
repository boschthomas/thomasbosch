...
\item @Reviewer 4: If reasoning can be performed or if it does not make any sense to use it depends on the contraint type to be expressed and not on the constraint language. Although, reasoning is not yet implemented for other constraint languages than OWL 2, these constraint languages can easily be extended.
...
subsection 3.3. The first paragraph is confusing. Refer to what? Follows? From this point on, the quality of the text decreases in general a bit...
...
1. The paper claims to study constraints for RDF data validation but the metrics used in the paper are not specific to data validation.




3. The term “expressivity scores” might be useful but a stronger argument is needed.
...
While gross numbers give some insight, if the paper listed which
problems can be solved by each constraint language, it might allow
readers more insight. 

Is there a useful grouping of the kinds of
restrictions that can be stated by each language?  

How would a user go
about choosing the right language for validation, or make trade offs
around the kinds of validation they need (I appreciate the authors
anticipate building a tool to help decide this)?
...
\begin{itemize}
	\item motivation
	\begin{itemize}
		\item 	A better angle for this paper would be to ask the administrator to point out among the positive constraints (such the PIs) which ones should be used for completing data by reasoning, and which ones should be considered as constraints about the data completeness that he/she wants to enforce. These constraints could be transformed into SPARQL unsat queries and checked over the data once completed using reasoning.
		...
		\item paper should provide a basis for future constraint language developments.
	\end{itemize}
...
While we evaluate expressivity scores, we use two sets of constraint types as before: $C_R$ and $\overline{C_R}$. The main motivation in that is, one may want to perform reasoning before actually validating the data.  For the sake of simplicity, we abbreviate \emph{expressing $C_R$} as $Expr(C_R)$, and \emph{expressing $\overline{C_R}$} as $Expr(\overline{C_R})$.
...
An interesting results is that in terms of expressivity on constraint types, 
