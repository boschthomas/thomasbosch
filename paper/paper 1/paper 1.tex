...
where first use cases for RDF constraint formulation and data validation have been discussed. 
...
The \emph{Web Ontology Language} \emph{(OWL)} in its current version \emph{OWL 2} is also used as a constraint language. 
...
Within the \emph{DCMI} working group, we initiated a collaboratively curated database of RDF validation requirements which contains the findings of the working groups based on various case studies provided by data institutions \cite{BoschEckert2014}. 
...
since the combination of \emph{DL} constructs needed to express a constraint type determines its computational complexity (Section \ref{reasoning}).
For this reason, the appendix of this paper contains mappings to \emph{DL} to determine which \emph{DL} constructs are needed to express each constraint type.
...
Using these findings, we are able to determine for the five most common constraint languages which constraint types they enable to express (see Table \ref{tab:constraint-type-specific-expressivity}) 
...
The fact that all $\mathcal{R}$ and $\overline{\mathcal{R}}$ constraint types are representable by \emph{SPIN} 
...
Another important role may play \emph{OWL 2 DL} with which 2/3 of all, 
nearly 1/2 of the $\overline{\mathcal{R}}$, and
almost all $\mathcal{R}$ constraint types can be expressed.
Even though, some $\mathcal{R}$ constraint types correspond to \emph{OWL 2 DL} axioms,
we cannot use them directly to validate RDF data since \emph{OWL 2} reasoning and validation assume different semantics which may lead to differences in results. 
...
In practice, however, \emph{OWL 2} is well-spread and \emph{RDFS/OWL 2} constructs are widely used to tell people and applications about how valid instances should look like.
In general, RDF documents follow the syntactic structure and the semantics of \emph{RDFS}/\emph{OWL 2} ontologies which could therefore not only be used for reasoning but also for validation. 
...
For each constraint type we investigate how efficient in terms of runtime validation is performed with and without reasoning.
By mapping to \emph{DL} we get an idea of the performance of each constraint type in worst case,
...
This constraint can be represented by \emph{OWL 2 DL} by building an
...
The corresponding \emph{DL} statement {\small\ms{Book $\equiv$ $\forall$ subject.\{Computer-Science,Librarian ship\}}}
...
as all (except of one) $\mathcal{R}$ constraint types are already expressible either by \emph{OWL 2 QL} or \emph{OWL 2 DL}.
...
\emph{OWL 2} is based on the {\em non-unique name assumption} (\emph{nUNA}) whereas RDF validation requires that different names represent different objects ({\em unique name assumption} (\emph{UNA})). 
Although, \emph{DLs/OWL 2} do not assume \emph{UNA}, they have the constructs \emph{owl:sameAs} and \emph{owl:differentFrom} to state that two names are the same or different.
...
(1) $\mathcal{CWA}$ denotes the set of constraint types which are dependent on the \emph{CWA},
	i.e., the set of constraint types for which it makes a difference in terms of validation results if the \emph{CWA} or the \emph{OWA} is assumed.
...
For the constraint type {\em disjoint classes} (\emph{R-7}), it does not make any difference regarding validation results if the \emph{CWA} or the \emph{OWA} is taken,
...
For \emph{Functional properties} (\emph{R-57/65}), it makes a difference with regard to validation results if \emph{UNA} or \emph{nUNA} is assumed.
...
and which may be the same as the violating resource when \emph{nUNA} is assumed.

We evaluated for each constraint type 
if it is dependent on the \emph{CWA} and on the \emph{UNA} (for a detailed analysis we refer to the appendix of this paper \cite{BoschNolleAcarEckert2015}). 
...
Hence, for the majority of the constraint types it makes a difference in terms of validation results if the \emph{CWA} or the \emph{OWA} and if the \emph{UNA} or the \emph{nUNA} is assumed.
...
The \emph{SPIN} engine checks for each resource if it satisfies all constraints, which are associated with its classes, and generates a result RDF graph containing information about all constraint violations.
We provide implementations to validate constraints of all constraint types expressible by \emph{OWL 2 QL}, \emph{OWL 2 DL}, and \emph{DSP} 
...
A \emph{SPIN} construct template contains a \emph{SPARQL CONSTRUCT} query generating constraint violation triples which indicate the subject, the properties, and the constraint causing the violations and the reason why violations have been raised.
...
We implemented reasoning capabilities for all $\mathcal{R}$ constraint types for which \emph{OWL 2 QL} and \emph{OWL 2 DL} reasoning may be performed\footref{spin-mappings}.

\section{Related Work}
\label{sec:related-Work}

Tao \cite{tao2012integrity} suggested an \emph{OWL 2 DL} extension to support integrity constraints 
which enables to use \emph{OWL 2} as a constraint language for validation under the \emph{CWA} by conjunctive query answering. 
...
Although, the alternative semantics for \emph{OWL 2} is implemented in the \emph{Stardog} database\footnote{\url{http://stardog.com/}}, it has never been submitted to a standards organization such as the \emph{W3C}.
...
Validation and reasoning assume different semantics which may lead to different validation results when applied to particular constraint types. %when particular constraint types are used for validation and for reasoning. 
\emph{OWL 2} reasoning requires the \emph{OWA} with the \emph{nUNA}, 
whereas validation is based on the \emph{CWA} and the \emph{UNA}.
Therefore, we investigated for each constraint type
(1) if it depends on the \emph{CWA} and (2) if it depends on the \emph{UNA}.
For the majority of the constraint types, it makes a difference in terms of validation results if the \emph{CWA} or the \emph{OWA} and if the \emph{UNA} or the \emph{nUNA} is assumed (Section \ref{CWA-UNA-dependency}).
