...
Using these findings, we are able to determine for the most common constraint languages which constraint types they are able to express, i.e., we determine their constraint type specific expressivity.
Therefore, we evaluated the five most promising languages on fulfilling each requirement to formulate constraints (see appendix of this paper) \cite{BoschNolleAcarEckert2015}.
...
We introduce the intuitive notion of \emph{expressivity scores} rather than classical \emph{expressivity} of languages
...
(1) expressing the 32 constraint types for which reasoning may be performed before actually validating the data as $Expr(\mathcal{R})$ and 
(2) expressing the 42 constraint types for which reasoning does not improve data quality in any obvious sense as $Expr(\overline{\mathcal{R}})$.
...
\begin{table}[H]
	\centering
      ...
      %\textbf{Expressivity Class} & $Expr(\overline{\mathcal{CT}_R})$ (42) & $Expr(\mathcal{CT}_R)$ (32) \\		
			\textbf{} & $Expr(\overline{\mathcal{R}})$ (42) & $Expr(\mathcal{R})$ (32) \\	
      \hline
			\emph{DSP} & 6 & 8 \\
