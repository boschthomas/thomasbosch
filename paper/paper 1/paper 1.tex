...
Table \ref{tab:constraint-type-specific-expressivity-absolute-values} contains the absolute numbers of supported constrain types, 
Table \ref{tab:constraint-type-specific-expressivity-percentage-values} appropriate percentage values.
...
Percentage values include constraint types supported by means of limitations, workarounds, or extensions.   
...
The following order is derived from the constraint type specific expressivity of constraint languages - including limitations, workarounds, and extensions since constraint types are still expressible:
{\tiny
\begin{eqnarray*}
%Expr(\mathcal{C}): ... \\
...
Furthermore, the fact that all (1) constraint types with reasoning and (2) constraint types without reasoning are representable by \emph{SPIN} (if we ignore the last constraint type \emph{recursive queries} which is partly implemented in \emph{SPIN}) 
