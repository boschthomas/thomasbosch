...
Reasoning in Semantic Web refers to logical reasoning that  makes implicitly available knowledge explicitly available.
...
Hence, we overcome this weakness of redundancy by associating the constraint with the super-class \emph{publication}.
...
If reasoning can be performed or if it does not make any sense to use it depends on the individual constraint type to be expressed and not on the language. 
Although, reasoning is not yet implemented for other languages than \emph{OWL 2}, languages can easily be extended in form of SPARQL queries.

Users should decide which constraints to use for completing data by reasoning 
and which ones should be considered as constraints about data accuracy and completeness which could be checked over the data once completed using reasoning.
Thus, we investigated for each constraint type if reasoning can be performed prior to validation to enhance data quality.

We examine the effects of reasoning on the computational complexity of constraint types...
...
(3) we classify RDF constraint types according to CWA/UNA dependency, reasoning, and computational complexity (Section \ref{classification-rdf-constraints}).
...
As we know that \emph{books} can only have \emph{persons} as \emph{authors} ({\small \ms{Book} \ms{$\sqsubseteq$} \ms{$\forall$} \ms{author} \ms{.} \ms{Person} }), 
...
But if we apply a reasoner before validating, this constraint violation is resolved since the implicit triple {\small\ms{Person(Mark-Twain)}} is inferred. 
...
With its open-world semantics, used as a constraint language, \emph{OWL 2} solves the redundancy problem (see introduction). 
...
\emph{OWL 2} is based on the {\em non-unique name assumption} (\emph{nUNA}) whereas RDF validation requires that different names represent different objects (a.k.a. {\em unique name assumption} (\emph{UNA})). 
...
Reasoning in OWL 2 is based on the semantics of {\em open-world assumption} (\emph{OWA}), i.e., a statement cannot be inferred to be false if it cannot be proved to be true  which fits its primary design purpose: to represent knowledge on the World Wide Web. 
...
As each \emph{book} must have a title ({\small\ms{Book $\sqsubseteq \exists$ title.$\top$}}) and {\em Hamlet} is a book ({\small\ms{Book(Hamlet)}}),
...
On the other hand, many RDF validation scenarios require the {\em closed-world assumption} (\emph{CWA}) (i.e., a statement is inferred to be false if it cannot be proved to be true).
Hence, classical constraint languages are based on CWA where constraints need to be satisfied only by named individuals which yields in return to a constraint violation. This would be a shortcoming of \emph{OWA} in RDF validation. 
We can further observe that there is a clash caused by the functional property {\em title} ({\small\ms{funct (title)}}) and the \emph{UNA},
...
%\end{itemize}
For this paper, we identified in total 74 requirements to formulate RDF constraints; each of them corresponding to a constraint type.\textcolor{red}{ToDO: 74 - 82} 
...
The subsequent steps are needed to get an idea about the performance (i.e. computational complexity) to validate simple (atomic) and complex (composed) constraint types:
\begin{enumerate}
	...
	\item When combining reasoning and validation, 
	the distinction of constraint types regarding the dependency on the CWA and
  on the UNA is an important aspect.
  Thus, we evaluate each constraint type regarding the CWA and UNA dependency.
	\item  Since reasoning makes difference  for some validation types, we note them with the computational complexity  of  the language that they are expressed in.
	\item Since the combination of DL constructs of a constraint type determines its complexity, by mapping to DL we get an idea of the complexity for each constraint type in worst case.
\end{enumerate}
...
As many \emph{OWL 2} axioms correspond to constraint types we want to use for validation, 
...
{\em Minimum qualified cardinality restrictions} (\emph{R-75}) is a $\mathcal{CT}_{CWA}$ constraint type, i.e., depends on the \emph{CWA}.
\emph{Publications}, e.g., must have at least one \emph{author} (\ms{Publication $\sqsubseteq$ $\geq$1 author.Person}).
...
The property \emph{title} is functional (\ms{funct (title)}), i.e., for each \emph{book} there can be at most one distinct \emph{title}.
...
The {\em existential quantification} (\emph{R-86}) \ms{Publication $\equiv$ $\exists$ publisher.Publisher}, e.g., restricts \emph{publications} to have at least one \emph{publisher}.
...
{\em Literal Pattern Matching} (\emph{R-44}), restricting literals to match given patterns,
...
{\em Sub-Properties} (\emph{R-54/64}), e.g., 
...
If we use \emph{sub-properties} without reasoning and the data contains the triple \ms{editor(A+Journal} \ms{Volume, A+Editor)},
then the triple \ms{creator(A+JournalVolume, A+Editor)} has to be stated explicitly.
If this triple is not stated, a constraint violation occurs.
...
\ms{Publication} \ms{$\equiv$} \ms{$\exists$} \ms{publisher.} \ms{Publisher} restricts publications to have at least one publisher - which can be expressed in OWL 2 QL:
\begin{ex}
[ a owl:Restriction ;
...
\end{ex}
If we execute reasoning on data containing the triples \ms{publisher(A+Conference} \ms{Proceedings, A+Publisher)} and \ms{rdf:} \ms{type (A+Publisher, Publisher)}, 
it can be inferred that {\em A+ConferenceProceedings} is a {\em publication}.
Now, all constraints associated with {\em publications} are also validated for the individual {\em A+Conference} \emph{Proceedings} (e.g. that \emph{publications} must have at least one \emph{author}).
Without reasoning, constraints on {\em publications} are not validated as \emph{A+Conference} \emph{Proceedings} is not part of the class extension of {\em publication}.
...
For the majority of the constraint types, reasoning is not needed to be performed prior to validation. \textcolor{red}{unclear!}
...
reviews:
\begin{itemize}
	\item However, if it is about the “complexity on reasoning”, as we do mean ontological reasoning  here, the discussion is done according to well-established complexity results (in literature) on reasoning in particular DL languages, having only those  features used in constraints. Therefore the complex interferences are included. 
	\item The first paragraph is confusing. Refer to what? Follows? 
\end{itemize}

To have a further evaluation on the latter and to refer easily, we divide $\mathcal{C}_R$ into two classes (not disjoint). 
...
\hline
$\overline{\mathcal{C}_R}$ & \textsc{Pspace}-Complete \\[7pt] 
\hline
$\mathcal{C}_R ^{\mathcal{QL}}$ & \textsc{Ptime} \\[7pt]
\hline
$\mathcal{C}_R ^{\mathcal{DL}}$ & \textsc{N2exptime} \\[7pt]
...
Note that there is also OWL 2 Full (which is not a DL) for reasoning that we will not include in our classification, since its high worst case complexity (undecidability) as well as interesting RDF constraint types with reasoning is already available in aforementioned OWLs (OWL 2 DL/QL).
...
reviews:
\begin{itemize}
	\item The term “expressivity scores” might be useful but a stronger argument is needed.
\end{itemize}
...
reviews:
\begin{itemize}
	\item logical semantics vs. notation
	
	\begin{itemize}
			\item 
This paper makes a confusion between notations and logical semantics.
...
%restrictions that can be stated by each language?  
	\item A reader might want to see some lessons or experiences that could be learned from such a comparative study.
\end{itemize}
