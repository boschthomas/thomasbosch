...
Their relevance varies with respect to several foremost criteria in validation process, such as user-friendliness (how easy is it for a user to define constraints?), constraint expressivity (what kind of constraints can be expressed?) and performance (how efficient in terms of runtime can RDF be validated?). 
Furthermore, the service of reasoning and its usability is in question. 

-----

explain validation (example)
examples:
- check if pflichtfelder belegt sind
- strings müssen einem komkretem pattern entsprechen

describe reasoning in general

why reasoning for validation?

was hat reasoning mit validierung zu tun? --> avoid redundancy (example)
...
A library wants to publish some data sets as
...
basic constraints to achieve high data quality:



The major shortcoming when validation without reasoning is redundancy



All publications must have a publishing date.
As books, conference proceedings, and journal articles are publications, we do not have to state explicitly that publishing dates must be stated for each publication sub class.

As we defined that each publication must have at least 1 creator and books are publications, books must also have at least one creator.
Therefore, we need to define authors as well as creators for books with redundant values.
If we define author to be a sub property of creator and we relate a book to an author, the creator relationship can be inferred by performing reasoning.
As conference and journal proceedings are publications, we can also define the property editor as sub property of creator
...
\begin{itemize}
  ...
  \item Journal of web semantics articles have the editors Finin, Staab, Horrocks, and Bernstein
	...
	\item ISBN identifiers must conform to specific patterns
\end{itemize}

%conference proceedings contain articles / erschienen in
%conference date
%proceedings herausgegeben von Verlag

-----

library domain published their data in XML for a long time 

sehr feine schemas 

parallel to XML world

viele anwendungsszenarien

klare beschreibung der daten

publication of closed data
...
contributions:
- diskussionsgrundlage schaffen für validierung in rdf
- rolle herausarbeiten von reasoning für validierung / why is reasoning relevant for vaidation?
- reasoning wift fragen bzgl der effekte typischer constraints auf die komplexität auf und wie wird das von aktuwllen sprachen unterstützt
- create the ground
- 2 welten zusammenbrungen (validation and reasoning) / zusaamenhang dieser communities
- technical contribution: investigation of constraint languages regarding the role of reasoning for validation

we addree the fowlling audieneces / zielgruppen)
1. metadata practicioners seaking for the role of reasoning for validation
2. reasoning experts to raise the awareness for practical applications of reasoning and validation

-----

\tb{replace current examples by following examples}
...
we map constraints to dl to investigate the expressivity of constraintlanguages
explain dl syntax and knowledgebase / short example to explain simple constraints / explain why mapping to dl is used 
...
\tb{I would rather use the title 'The Role of Reasoning for Validation'}
...
\tb{if we classify constraints according to CWA / OWA, we shift this section to subsection in next chapter}
...
We recently published a technical report\footnote{Available at: \url{https://github.com/boschthomas/RDF-validation-research/paper}} (serving as appendix of this paper) in which we explain each requirement in detail and in which we give examples for each (represented by listed constraint languages)\tb{citation}.
This technical report also contains mappings to DL to logically underpin each requirement and to determine which DL constructs are needed to express which constraint type.
...
In this section, we show how to determine the performance (the computational complexity) to validate simple (atomic) and complex (composed) constraint types.
\begin{enumerate}
	\item As reasoning may be performed as a pre-validation step, we determine for each constraint type if it makes sense to perform reasoning for this constraint type.
	\item As validation may be performed without reasoning, but also by means of different reasoning types, we assign each constraint type to a validation type (which is associated with a specific complexity class).
	\item The combination of DL constructs of a constraint type determines its complexity. By mapping each constraint type to DL and as the complexity of DL constructs are known (some DL constructs are only expressible by particular OWL 2 profile which are associated with a complexity class), we get an idea of the complexity for each constraint type in worst case.
\end{enumerate}

\subsection{CWA vs. OWA / Semantics}

\tb{just inserted Christain's feedback so far}

classification of constraints according to CWA and UNA
- CWA dependent constrains
- CWA independent constraints
-----

The UNA and CWA are important aspects, however, in Section 4 nd 5
...
\subsubsection{Constraints with Reasoning}
...
{\em Subsumption}\footnote{corresponds to R-100-SUBSUMPTION}
...
The {\em Property Domain}\footnote{corresponds to R-25-PROPERTY-DOMAIN} constraint
...
The {\em Existential Quantification}\footnote{corresponds to R-86-EXISTENTIAL-QUANTIFICATION-ON-PROPERTIES}
...
\textbf{OWL 2 DL Reasoning.}
{\em Universal quantifications}\footnote{corresponds to R-91-UNIVERSAL-QUANTIFICATION-ON-PROPERTIES}
...
\subsubsection{Constraints without Reasoning}
...
{\em Allowed Values}\footnote{corresponds to R-30-ALLOWED-VALUES}.
...
{\em Context-Specific Exclusive OR of Properties}\footnote{corresponds to  R-11-CONTEXT-SPECIFIC-EXCLUSIVE-OR-OF-PROPERTIES}.
...
{\em Default Values}\footnote{Corresponds to R-31-DEFAULT-VALUES-OF-RDF-OBJECTS and R-38-DEFAULT-VALUES-OF-RDF-LITERALS}.
...
\tb{1/2 sentences how reasoning is implemented in the validator}

\tb{Andy: 1 sentence about query expansion}
