...
- We denote the whole set of constraint types with $\mathcal{CT}$
- percent values include workarounds...
...
Using these findings, we are able to determine for the five most common constraint languages which constraint types they are able to express, i.e., we evaluate their constraint type specific expressivity (the detailed evaluation is described in the appendix of this paper) \cite{BoschNolleAcarEckert2015}.
...
\begin{table}[H]
	\centering
	  %\caption{Constraint Type Specific Expressivity}
		\caption{Overview on the Expressivity of Constraint Languages (Absolute Values)}
	  \scriptsize
...
\begin{table}[H]
	\centering
	  %\caption{Constraint Type Specific Expressivity}
		\caption{Overview on the Expressivity of Constraint Languages (Percentage Values)}
	  \scriptsize
		\begin{tabular}{l|c|c|c}
      %\textbf{Expressivity Class} & $Expr(\overline{\mathcal{CT}_R})$ (42) & $Expr(\mathcal{CT}_R)$ (32) \\		
			\textbf{} & $\mathcal{CT}$ (81) & $\overline{\mathcal{R}}$ (46) & $\mathcal{R}$ (35) \\	
...
The following order is derived from the expressivity scores of constraint languages - including limitations, workarounds, and extensions since constraint types are still expressible:
{\tiny
\begin{eqnarray*}
%Expr(\mathcal{C}): ... \\
