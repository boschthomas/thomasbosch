...
(1) $\mathcal{R}$ is the set of constraint types for which reasoning may be performed prior to validation (especially when not all the knowledge is explicit) in order to enhance data quality
...
(2) $\overline{\mathcal{R}}$  denotes the complement that is the set of constraint types for which reasoning cannot be done or 
for which reasoning does not improve data quality in any obvious sense.

{\em Sub-Properties} (\emph{R-54/64}) is an $\mathcal{R}$ constraint type.
...
is an $\overline{\mathcal{R}}$ constraint type:
...
$\mathcal{R}$ is the set of constraint types for which reasoning can be performed prior to validation 
in order to enhance data quality either by resolving violations or by causing additional useful violations.
We distinguish between two disjoint sub sets of $\mathcal{R}$:
