...
To investigate the affect of reasoning to the validation process, we grouped constraints under two groups

\begin{itemize}
	\item $\overline{\mathcal{C}_{\overline{R}}}$: contraints without reasoning
	\item $\mathcal{C}_R$: contraints with reasoning
\end{itemize}

We call the whole set of constraint as $\mathcal{C}$. 
...
To evaluate RDF validation requirements, we had the classes of constraints: \emph{constraints without reasoning} and \emph{constraints with reasoning}. In addition, to have a further evaluation on the latter and to refer easily, we divide it into two classes (not disjoint). Overall, we have the following. Since, OWL 2 DL is more expressive than OWL 2 QL,  $\mathcal{C}_R ^{\mathcal{QL}} \subseteq \mathcal{C}_R ^{\mathcal{DL}}$ follows. 
...
Expressivity scores (on how many of those constraints can be expressed by a particular constraint language):

\begin{itemize}
  %\item $Expr(\mathcal{C})$: set of all constraints
	\item $Expr(\overline{\mathcal{C}_R})$: set of constraints without reasoning 
	\item $Expr(\mathcal{C}_R)$: set of constraints with reasoning
\end{itemize}
...
Classical expressivity (in computer science and logic):
\begin{itemize}
	\item Expr(DSP): set of constraints expressible by DSP
	\item Expr(OWL2-DL): set of constraints expressible by OWL2-DL
	\item Expr(OWL2-QL): set of constraints expressible by OWL2-QL
	\item Expr(ReSh): set of constraints expressible by ReSh
	\item Expr(ShEx): set of constraints expressible by ShEx
	\item Expr(SPIN): set of constraints expressible by SPIN
\end{itemize}

Partial orderings are:
\begin{eqnarray*}
Expr(DSP): DSP \succ
\end{eqnarray*}
