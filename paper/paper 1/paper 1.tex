...
\keywords{RDF Validation, RDF Constraints, OWL 2, Reasoning, RDF Validation Requirements, Linked Data, Semantic Web} % NOT required for Proceedings
...
We distinguish 82 requirements to formulate constraints; 
each of them corresponding to a constraint type.
The \emph{W3C} WG defines \emph{constraint} as a component of a schema what needs to be satisfied\footnote{\url{https://www.w3.org/2014/data-shapes/wiki/Glossary}.}. 
A concrete constraint is instantiated from exactly one \emph{constraint type}.
...
%
%-----
\emph{OWL} is an expressive language which is mainly praised for its reasoning capabilities in the Semantic Web community. 
The knowledge representation formalism {\em Description logics} (\emph{DL}), with its  well-studied theoretical properties, provides the foundational basis for \emph{OWL}. 
...
We map constraint types to \emph{DL} statements to classify each constraint type according to semantics, reasoning, and expressivity
and to determine the capabilities of constraint languages with regard to the same characteristics.

As \emph{OWL 2} has different semantics than RDF validation and as we want to use \emph{OWL 2} axioms as constraint types, 
we determine the semantics in terms of validation results for each constraint type.
...
Therefore, we investigate the affect of reasoning to the validation process of each constraint type, i.e.,
...
The purpose of this paper is to provide a basis for the development of constraint languages.
...
To gain a better understanding about the role of reasoning for data quality and 
to direct the further development of constraint languages, we investigated ...
...
(2) We work out the role that reasoning plays in practical data validation and why reasoning is important for validation (Section \ref{the-role-of-reasoning-for-rdf-validation}).
%and therefore bridge the gap between these two worlds,
(3) we classify constraint types according to CWA/UNA dependency, reasoning, and computational complexity (Section \ref{classification-rdf-constraints}).
(4) We determine the constraint type specific expressivity of constraint languages (Section \ref{classification-constraint-languages}).
...
%\end{itemize}
\emph{OWL} in its current version \emph{OWL 2} is an expressive language which is based on Description logics, offering knowledge representation and reasoning services. 
Validation, however, is not the primary purpose of its design. 
This has lead to claims that \emph{OWL 2} cannot be used for validation. 
...
In practice, \emph{OWL 2} is well-spread and \emph{RDFS}/\emph{OWL 2} constructs are widely used to tell people and applications about how valid instances should be created.
\emph{RDF} documents follow the structure of \emph{RDFS}/\emph{OWL 2} ontologies which can therefore be reused for validation according to this structure. 
...
\begin{DL} 
$\mathcal{K}=\{$ \\ 
 \ms{ Book $\sqsubseteq \exists$ title.$\top$}, \ms{Book $\sqsubseteq \forall$ author.Person }\\
 \ms{ ( funct (title) )},\\
...
(1) If we apply reasoning before validating, the violation is resolved since the implicit triple {\small\ms{Person(Mark-Twain)}} is inferred. 
(2) Reasoning may cause additional violations needed to enhance data quality.
As \emph{books} are \emph{publications} {\small\ms{(Book $\sqsubseteq$ Publication),}} constraints on \emph{publications} are also validated for \emph{books} (e.g. each \emph{publication} must have a \emph{publisher}) which may result in further violations.
(3) The major shortcoming of classical constraint languages is \emph{redundancy} which is solved by \emph{OWL 2} with its open-world semantics (see Section \ref{introduction}). 
...
As each \emph{book} must have a \emph{title} {\small\ms{(Book $\sqsubseteq \exists$ title.$\top$)}} and {\em Hamlet} is a book {\small\ms{(Book(Hamlet)),}}
...
\emph{OWA} would be a shortcoming in RDF validation.  
...
Classical constraint languages are based on \emph{CWA} where constraints need to be satisfied only by named individuals which yields in return to a violation as there is no explicitly defined \emph{title}. 
...
\emph{OWL 2 QL} is based on the \textit{DL-Lite} family of description logics \cite{Artale2009,Calvanese2007}.
...
As 34 of 74 constraint types correspond to \emph{OWL 2} axioms we want to use for validation, 
we investigate their behavior in terms of validation results in different semantic settings. 
\emph{OWL 2} reasoning requires the OWA with the \emph{nUNA}\footnote{While \emph{DLs/OWL} do not assume \emph{UNA}, they have a construct (\emph{owl:sameAs}) to express when two names are the same or not.
...
When assuming \emph{OWA} and \emph{nUNA} for RDF validation, validation won’t be that restrictive and therefore we won’t get the intended validation results.
...
a \emph{publication} without an explicitly stated \emph{author} does not raise a constraint violation 
as the axiom entails that there must be an \emph{author} which we may not know.
(2) $\overline{\mathcal{CT}_{CWA}}$ is the complement of $\mathcal{CT}_{CWA}$ and thus includes constraint types which are independent on the \emph{CWA}.
...
as if there is a \emph{publication} which is a \emph{book} and a \emph{journal article} a constraint violation is raised in both settings.
%as all information needed for validation is explicitly available in both cases. 
...
(3) $\mathcal{CT}_{UNA}$ denotes the set of constraint types which are dependent on the \emph{UNA}.
...
which resolves the constraint violation. 
(4) $\overline{\mathcal{CT}_{UNA}}$ (complement of $\mathcal{CT}_{UNA}$) denotes the set of constraint types which are independent on the \emph{UNA}.
...
If \emph{A+ConferenceProceedings} are published by \emph{A+Publisher} and \emph{APlusPublisher},
it does not matter if \emph{A+Publisher} and \emph{APlusPublisher} refer to one identical \emph{publisher} or to two distinct \emph{publishers}
as in both cases the \emph{publication} has a \emph{publisher} which satisfies the constraint.

For this work, following the similar fashion, we evaluated for each constraint type 
...
The result of this evaluation is that we distinguish between XX $\mathcal{CT}_{CWA}$ and XX $\overline{\mathcal{CT}_{CWA}}$ and between XX $\mathcal{CT}_{UNA}$ and XX $\overline{\mathcal{CT}_{UNA}}$ constraint types.
Hence, for the majority of constraint types it makes a difference if the \emph{CWA} or the \emph{OWA} and if the \emph{UNA} or the \emph{nUNA} is assumed.
\textcolor{red}{ToDo: evaluation}
...
(1) $\mathcal{CT}_R$ is the set of constraint types for which reasoning can be performed prior to validation (especially when not all the knowledge is explicit) in order to enhance data quality
...
(2) $\overline{\mathcal{CT}_{R}}$  denotes the complement that is the set of constraint types for which reasoning cannot be done or 
for which reasoning does not improve data quality in any obvious sense.

{\em Sub-Properties} (\emph{R-54/64}) is an example of a $\mathcal{CT}_R$ constraint type for which reasoning can be performed prior to validation to enhace data quality.
%corresponds to axioms in \emph{OWL 2}
If a \emph{journal volume}, e.g., has an {\em editor} relationship to a \emph{person}, 
...
If we use \emph{sub-properties} without reasoning and the data contains the triple {\small\ms{editor(A+JournalVolume, A+Editor)}},
then the triple {\small\ms{creator(A+JournalVolume, A+Editor)}} has to be stated explicitly.
...
{\em Literal Pattern Matching} (\emph{R-44}), which restricts literals to match given patterns,
is an example of a $\overline{\mathcal{CT}_R}$ constraint type:
...
$\mathcal{CT}_R$ is the set of constraint types for which reasoning can be performed prior to validation 
in order to enhance data quality either by resolving violations or by causing additional useful violations.
We distinguish between two disjoint sub sets of $\mathcal{CT}_R$:
(1) Constraint types for which \emph{OWL 2 QL reasoning} can be performed as they are expressible by \emph{OWL 2 QL}.
(2) Constraint types for which \emph{OWL 2 DL reasoning} can be executed as \emph{OWL 2 QL} is not expressive enough to represent these constraint types \cite{owl2profiles2008}.
...
if it is not explicitly stated that \emph{Lewis-Carroll} is a \emph{person}. 
With reasoning, on the contrary, the class assignment {\small\ms{rdf:type(Lewis-Carroll, Person)}} is inferred which prevents the violation to be raised.
...
Reasoning may cause valuable violations which increase data quality.
...
Without reasoning, in contrast, the fact that \emph{A+Conference-Proceedings} is a publication is not explicit
...
$\overline{\mathcal{CT}_{R}}$ is the set of constraint types for which reasoning cannot be done or 
for which reasoning does not improve data quality in any obvious sense.
...
it can be defined that an individual can either have property A or property B, but not both.
...
\end{DL} 
\end{center}
Representing this constraint in \emph{OWL 2 DL} can be seen as a workaround 
...
CC2 rdfs:subClassOf [ a owl:Restriction ;
    ...
    owl:someValuesFrom xsd:string ] .
\end{ex}
The same constraint can be represented by \emph{ShEx} more intuitively and concisely:
\begin{ex}
...
The majority of the $\overline{\mathcal{CT}_{R}}$ constraint types can neither be expressed by \emph{OWL 2 QL} nor by \emph{OWL 2 DL}, 
but by other constraint languages like \emph{DSP}, \emph{ShEx}, \emph{ReSh}, or \emph{SPIN}.
...
default values for keywords like {\small\ms{keyword(This-Paper,"Semantic Web")}} are inferred.
...
We divide $\mathcal{CT}_R$ into two not disjoint sets of constraint types: 
$\mathcal{CT}_R ^{\mathcal{QL}} \subseteq \mathcal{CT}_R ^{\mathcal{DL}}$ (\emph{OWL 2 DL} is more expressive than \emph{OWL 2 QL}).
According to the execution of reasoning prior to validation, we distinguish three different types of validation: 
\begin{enumerate}\itemsep0.5em
	\item $\overline{\mathcal{CT}_R}$ denotes the set of constraint types for which validation is performed by query answering without reasoning.
	\item $\mathcal{CT}_R ^{\mathcal{QL}}$ is the set of constraint types which are expressible by \emph{OWL 2 QL} and for which validation is executed by query answering with \emph{OWL 2 QL} reasoning.
	\item $\mathcal{CT}_R ^{\mathcal{DL}}$ stands for the set of constraint types which are expressible by \emph{OWL 2 DL} and for which validation is run by query answering with \emph{OWL 2 DL} reasoning.
\end{enumerate}
We limit $\mathcal{CT}_R$ to the set of constraint types which can be expressed by \emph{OWL 2 DL} %and still require reasoning 
($\mathcal{CT}_R$ = $\mathcal{CT}_R ^{\mathcal{DL}}$), as \emph{OWL 2 DL} is still quite expressive.
...
Table \ref{tab:constraint-type-specific-complexity-classes} gives an overview over the complexity of RDF validation when reasoning is performed and when reasoning is not performed grouped by validation type.
...
\hline
$\overline{\mathcal{CT}_R}$ & \textsc{Pspace}-Complete \\[7pt] 
\hline
$\mathcal{CT}_R ^{\mathcal{QL}}$ & \textsc{Ptime} \\[7pt]
\hline
$\mathcal{CT}_R ^{\mathcal{DL}}$ & \textsc{N2exptime} \\[7pt]
\end{tabular}
\label{tab:constraint-type-specific-complexity-classes}
\end{table}
RDF validation with or without reasoning corresponds to performing \emph{SPARQL} queries.  
It is known that performing \emph{SPARQL} queries is carried out in \textsc{Pspace}-Complete \cite{Perez2009}. 
As \emph{OWL 2} profiles are based on the \textit{DL-Lite} family and query answering in \emph{OWL 2 QL} (which is based on \textit{DL-Lit}$e_R$) is performed in \textsc{LogSpace} (or rather in \emph{AC$^0$}) \cite{Calvanese2007}, the same complexity class applies for
...
With the more expressive profile \emph{OWL 2 DL}, reasoning is executed in \textsc{N2exptime} \cite{owl2profiles2008} which is a class of considerably higher complexity.
As we consider ontological reasoning, complexity classes are assigned to validation types 
according to well-established complexity results on reasoning of \emph{DL} languages.
...
In this section, we concern ourselves with the expressivity of constraint languages, or constraint type (requirement) specific expressivity.
Therefore, we evaluated the five most popular constraint languages on fulfilling each requirement to formulate constraints.
We also evaluated if a specific requirement is expressed by \emph{OWL 2 QL} or if the more expressive \emph{OWL 2 DL} is needed \cite{BoschNolleAcarEckert2015}.
...
In evaluating the expressivity,  we introduce the intuitive notion of \emph{expressivity scores} rather than classical expressivity. The main motivation in that is, the list of constraint types is an updated list, and almost none (except \emph{SPIN}) of the constraint languages contains one another.
...
While we evaluate expressivity scores, we use two sets of constraint types as before: $\mathcal{CT}_R$ and $\overline{\mathcal{CT}_R}$. 
The main motivation in that is, that one may want to perform reasoning before actually validating the data.  For the sake of simplicity, we abbreviate \emph{expressing} $\mathcal{CT}_R$ as $Expr(\mathcal{CT}_R)$, and \emph{expressing} $\overline{\mathcal{CT}_R}$ as $Expr(\overline{\mathcal{CT}_R})$.
...
Although  \emph{OWL 2}  is the only implementation (which can be used/considered as a constraint language) with reasoning features, 
some constraint types in $\mathcal{CT}_R$ are not only expressible by \emph{OWL 2}  but also by other constraint languages. 
...
\begin{table}[H]
	\centering
	  ...
	  \scriptsize
		\begin{tabular}{l|c|c}
      \textbf{Expressivity Class} & $Expr(\overline{\mathcal{C}_R})$ (42) & $Expr(\mathcal{C}_R)$ (32) \\		
      \hline
			DSP & 6 & 8 \\
			OWL2-DL & 18(4) & 29 \\ 
			OWL2-QL & 7(4) & 11 \\
...
Numbers in parentheses in the other columns indicate the number of constraint types which may be expressed by that particular constraint language - either by limitations, workarounds, or extensions.  
...
The following order is derived from expressivity scores:
{\tiny
\begin{eqnarray*}
%Expr(\mathcal{C}): ... \\
...
If, e.g., many constraint types in $\mathcal{CT}_R$ are needed, the constraint language \emph{SPIN} is the most expressive one.
On the other hand, if only specific requirements in $\overline{\mathcal{CT}_R}$ or $\mathcal{CT}_R$ are needed, 
it can be more reasonable to use a more compact one like \emph{ShEx}.

For a validation labourer, it is relevant to question the expressivity interrelation between those constraint languages, e.g., "which constraint language could contain the whole \emph{DSP} or  could be used instead?". This can also maintain the future development plans and directions of those languages in the way of achieving to be a solid standard constraint language.

With the aforementioned concern, let us introduce \emph{Expr(DSP)} which denotes the set of all constraint types which are expressible by \emph{DSP}.  We can consider \emph{Expr(DSP)} as an expressivity class for the constraint language \emph{DSP}. Using the same fashion, one can obtain an expressivity class  per constraint language, which denotes the set of constraint types which  are expressible by that constraint language.
%\begin{itemize}
...
If we rank them according to their scores, just as before we get the following as a brief picture:
%{\tiny
%\begin{eqnarray*} 
...
From Tables \ref{tab:expressivity-sores-according-to-expressivity-classes-1}-\ref{tab:expressivity-sores-according-to-expressivity-classes-2} (with numbers), one important finding is that in terms of expressivity on constraint types, 
all constraint languages can be fully covered by \emph{SPIN} if we ignore one last constraint type (i.e., the recursive queries which is partly implemented in \emph{SPIN}). 
Except for \emph{SPIN}, all supported constraint types are already implemented by particular constraint languages.
Yet another interesting message is that, none of other languages covers the other so far in terms of expressivity.
As we keep the validation requirements updated and those languages are further developed, it is likely that this list will change as well in the direction of needs of the validation community.
