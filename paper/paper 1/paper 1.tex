...
%\end{itemize}
OWL 2 is an expressive semantic web language which is based on description logics, offering knowledge representation and reasoning services. Validation, however, does not appear to be \tb{does not appear to be --> Kai mentioned this does not sound that scientific}a genuine part of it.  
...
As we know that books can only have persons as authors (\ms{Book $\sqsubseteq \forall$ author.Person}), 
...
Reasoning may cause additional constraint violations needed to enhance data quality, e.g., as books are publications constraint on publications are also validated for books (such as each publication must have a publisher).
...
OWL's semantics is also based on the {\em open-world assumption (OWA)}, i.e., a statement cannot be inferred to be false if it cannot be proved to be true  which fits its primary design purpose: to represent knowledge on the World Wide Web. 
...
As each book must have a title (\ms{Book $\sqsubseteq \exists$ title.$\top$}) and {\em Hamlet} is a book (\ms{Book(Hamlet)}),
