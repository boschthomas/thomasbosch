...
For many RDF applications\er{Do we mean software applications (i.e., RDF-based applications) or application as a task?}\tb{tasks not just RDF-based applications}, the formulation of constraints and the automatic validation of data according to these constraints is a much sought-after feature. 
...
Two WGs, \er{WG? Working groups?}\tb{working groups} that follow up on this workshop and address RDF constraint formulation and validation, are established in 2014: 
...
There are multiple constraint languages having different syntaxes and semantics which can be used to express RDF constraints e.g., \emph{at-least} restrictions.
The most promising constraint languages are \er{Promising on what? Or popular or common? This should be underlined. No aim yet  except expressing things has been mentioned .}
\tb{promising to be possible standard constraint languages / candidates as standard constraint languages}
...
However, there is no single standard all-purpose constraint language. \er{Notice that except expressivity, no purpose has been yet uttered.}
The choice depends on the individual use case. \er{I think before that the importance of complexity and expressivity in practice of rdf-validation should be briefly told in one or few sentence(s), validators (if wanted), as well as a closing short paragraph telling the structure/organisation of the paper would be good. Let us keep in mind that the introduction covers and extends the abstract for integrity.}
...
\end{itemize}
\er{I think after putting 2 pages of motivation, either we should get rid of this itemisation in terms of reshape or reorganise, or embed them inside flowing motivation text. They are so many. }
...
We added mappings to DL to logically underpin each requirement and to determine which DL constructs are needed to express each constraint.
...
\begin{itemize}
	\item \ms{C}: contraints without reasoning
	\item \ms{C-R}: contraints with reasoning
\end{itemize}

\ms{Constraints without reasoning} are constraints for which reasoning cannot be done or does not make any sense.

\textcolor{red}{ToDo Thomas: 1 example}

\ms{Constraints with reasoning} are constraints for which reasoning may be performed or is even needed.
\ms{Constraints with reasoning} correspond to axioms in DL - e.g. subsumption, sup properties, cardinality restrictions, existential, and universal restrictions.
...
See that this is not a subclass of constraint anymore. 
...
\ms{Sub Properties}, e.g., are constraints for which reasoning may be performed.
...
then the triple \ms{attackingByWeapon(Luke, DarthVader)} has to be stated in the data explicitly.
If this triple is not stated, a constraint violation is raised.
If we use sub properties with reasoning, the triple (causing a constraint violation) is inferred resolving the possible constraint violation.

\subsection{Complexity}

According to our evaluation of requirements to formulate RDF constraints, we identified four main ways of validating the two constraint classes \ms{constraints without reasoning} and \ms{constraints with reasoning} assigned to four RDF validation complexity classes:
...
The complexity class \ms{C-R} encompasses all \ms{constraints with reasoning} for which reasoning can be executed but cannot be expressed by OWL 2.  
...
As, e.g, the constraint \ms{cardinality shortcuts} is associated with the complexity class \ms{C-R} and OWL2 DL reasoning can be done on cardinality restrictions,
...
We evaluated to which extend the most promising five constraint languages fulfill each requirement to formulate RDF constraints.
...
we use the two types of constraints - \ms{Constraints without reasoning} and \ms{Constraints with reasoning} - as criteria to measure constraint-specific expressivity.
We define these two expressivity classes as follows: 

\begin{itemize}
	\item \ms{Expr-C}: constraints without reasoning
	\item \ms{Expr-R}: constraints with reasoning
\end{itemize}

\ms{Constraints with reasoning} can be used as axioms to infer new triples out of explicitly stated triples (see section \ref{sec:rdf-validation-requirements-and-reasoning}), but also to express constraints.
...
\ms{constraints with reasoning} cannot only be expressed by OWL 2. 
...
If, e.g., many \ms{constraints with reasoning} are needed, the constraint language SPIN is the most expressive one.
If, on the other side, only specific \ms{constraints without reasoning} or \ms{constraints with reasoning} are needed for particular use cases, 
...
The next table shows the amount of constraints (of both constraint classes) can be expressed by which constraint language - aggregated by complexity class and sorted by ascending complexity.

\begin{center}
\begin{constraint-languages-complexity}
\ms{C-C} (41) & 6 & 18(4) & 7(4) & 8 & 12 & 40(1)\\
\ms{C-R-OWL2-QL} (11) & 0 & 11 & 11 & 0(2) & 1 & 11 \\
\ms{C-R-OWL2-DL} (18) & 7 & 18 & 0 & 9 & 8(1) & 18 \\
\ms{C-R} (4) & 1 & 0 & 0 & 2 & 2 & 4 \\ 
\end{constraint-languages-complexity}
\end{center}

Partial (not necessarily total) orderings are:
\begin{eqnarray*}
\ms{C-C}: DSP \subset OWL2-QL \subset ReSh \subset ShEx \subset OWL2-DL \subset SPIN \\
\ms{C-R-OWL2-QL}: DSP \subset ReSh \subset ShEx \subset SPIN \subseteq OWL2-QL \subseteq OWL2-DL \\
\ms{C-R-OWL2-DL}: OWL2-QL \subset DSP \subset ShEx \subset ReSh \subset SPIN \subseteq OWL2-DL \\
\ms{C-R}: OWL2-QL \subseteq OWL2-DL \subset DSP \subset ReSh \subseteq ShEx \subset SPIN
\end{eqnarray*}
...
\textcolor{red}{universal quantification}
...
\tb{comment of Erman: there is also OWL 2 Full, which is not a DL at
all because its too expressive (therefore has the highest complexity
class: undecidable), but it HAS reasoning.  We do not mention it all.
Perhaps, we might at least mention it in a sentence that we will not
take it into our evaluation because it is this and this, impractical and
so on.}
