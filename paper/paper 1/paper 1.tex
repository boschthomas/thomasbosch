...
Hence, we determined for each constraint type if it makes sense to perform reasoning prior to validation.
...
The subsequent steps are needed to get an idea about the performance (i.e., the computational complexity) to validate \textcolor{red}{simple (atomic) and complex (composed) constraint types}:
\begin{enumerate}
  ...
  %on the UNA is an important aspect when combining reasoning and validation,
	...
	\item We determine for each constraint type if it makes sense to perform reasoning prior to validation
  in order to investigate the affect of reasoning to the validation process.
...
We determine for each constraint type if it makes sense to perform reasoning prior to validation
in order to investigate the affect of reasoning to the validation process.
The W3C WG defines \emph{constraint} as a component of a schema what needs to be satisfied\footnote{\url{https://www.w3.org/2014/data-shapes/wiki/Glossary}.}. 
...
(1) $\mathcal{CT}_R$ is the set of constraint types that can carry out the validation process via reasoning, especially when not all the knowledge is explicit.  
(2) $\overline{\mathcal{CT}_{R}}$  denotes the complement that is the set of constraint types for which reasoning cannot be done or does not improve the result in any obvious sense.
...
Note that there is also OWL 2 Full (which is not a DL) for reasoning that we will not include in our classification, since its high worst case complexity (undecidability) as well as interesting constraint types with reasoning is already available in aforementioned OWLs (OWL 2 DL/QL).
