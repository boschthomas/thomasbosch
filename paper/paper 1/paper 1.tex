...
RDF validation and reasoning assume different semantics which may lead to different results when particular constraint types are used for validation and for reasoning. 
Reasoning requires the {\em closed-world assumption} (\emph{OWA}) with the {\em non-unique name assumption} (\emph{nUNA}), 
whereas RDF validation is classically based on the {\em closed-world assumption} (\emph{CWA}) and the {\em non-unique name assumption} (\emph{UNA}).
...
since validation and reasoning results depend on the underlying semantics.
This is the reason why we investigate the behavior in terms of validation results depending on the \emph{CWA} and the \emph{UNA} for each constraint type (Section \ref{CWA-UNA-dependency}).
...
For \emph{OWL 2}, we differentiate between the sub-languages \emph{OWL 2 DL} and \emph{OWL 2 QL}, as they differ with regard to expressivity and efficiency in performance.
The table shows in percentage values (and absolute numbers in brackets) how many $\mathcal{CT}$, $\mathcal{R}$, and $\overline{\mathcal{R}}$ constraint types are supported 
...
how to overcome the major shortcomings when validating RDF data by performing reasoning prior to validation.
For each constraint type, we investigate the effect of reasoning to the validation process.
