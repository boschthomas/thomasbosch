...
We investigate the expressivity of constraint languages - more specifically the constraint type/requirement specific expressivity of constraint languages.
Therefore, we evaluated the five most promising languages on fulfilling each requirement to formulate constraints.
We also evaluated if constraint types are expressible by \emph{OWL 2 QL} or if the more expressive \emph{OWL 2 DL} is needed \cite{BoschNolleAcarEckert2015}.
...
The main motivation in that is, the list of constraint types is an updated list, and almost none (except \emph{SPIN}) of the languages contains one another.
With the expressivity score of a language, we measure how many constraint types can be expressed by that particular language.
...
some constraint types in $\mathcal{CT}_R$ are not only expressible by \emph{OWL 2}  but also by other languages. 
