...
We define two sets of constraints to investigate the affect of reasoning to the validation process:
\begin{itemize}
	\item $\overline{\mathcal{C}_{\overline{R}}}$: contraints without reasoning \tb{why overline R?}
	\item $\mathcal{C}_R$: contraints with reasoning
\end{itemize}
We call the whole set of constraints as $\mathcal{C}$. \tb{this paragraph is about constraint without reasoning. I would add the whole set of constraints to the list above}
\emph{Constraints without reasoning} are constraints for which reasoning cannot be done or does not improve the result in any obvious sense.
\ms{Literal Pattern Matching}\footnote{See R-44-PATTERN-MATCHING-ON-RDF-LITERALS.}, e.g., restricts literals to match given patterns.
