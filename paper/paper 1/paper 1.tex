...
It can be beneficial not using the most expressive constraint languages when additional rather subjective criteria are evaluated.
...
Even if the \emph{absolute expressivity score} ($Expr(\overline{\mathcal{C}_R})$) of SPIN is much higher than the one of ShEx (ShEx $\subset$ SPIN), 
the \emph{relative expressivity score} could be the same for ShEx and SPIN - in case the three constraints are covered by both.
...
%evaluation constraint languages according to complexity / ideas of Erman
