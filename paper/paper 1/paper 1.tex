...
Their relevance varies with respect to several foremost criteria in validation process, such as constraint expressivity (what kind of constraints can be expressed?), performance (how efficient in terms of runtime can RDF data be validated?), as well as intuitiveness and conciseness (how easy is it for a user to define constraints?). 
%Furthermore, the service of reasoning and its usability is in question. 

% RDF validation in the library domain
% -----
Libraries are used to publish data in form of XML documents for a long time.
...
Publications, e.g., must have publishing dates.
...
We can use reasoning before validating to overcome this shortcoming.
Reasoning is the process of determining what follows from what has been
stated. Reasoning ranges from simple (books are publications, {\em Foundations of Semantic Web Technologies} is a book, and therefore it is a publication) to the very complex. 
Reasoning can also recognize impossibilities 
...
If we define that publications must have at least one creator and books are publications, books must also have at least one creator.
Hence, we need to define authors as well as creators for books with redundant values.
...
As conference and journal proceedings are publications, we can also define editor as sub property of creator
