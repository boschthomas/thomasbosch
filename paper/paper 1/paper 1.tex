...
\begin{eqnarray*} 
\mathcal{K} &=  & \ms{Person} \sqsubseteq \exists \ms{hasFather.Person}, (\ms{funct (hasFather)}),\\
 && \ms{Person(Anakin)}, \ms{Person(Luke)},\\
 &&\ms{hasFather(Luke, Anakin)}, \ms{hasFather(Luke, Shmi)}\}
\end{eqnarray*}
...
\tb{ToDo: chapter 6 of Jiao Tao's dissertation - http://tw.rpi.edu/web/doc/JiaoTaoDissertation}
