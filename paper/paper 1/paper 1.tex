...
As we know that \emph{books} can only have \emph{persons} as \emph{authors} (\ms{Book $\sqsubseteq \forall$ author.Person}), 
{\em Huckleberry-Finn} is a \emph{book}  (\ms{Book(Huckleberry-Finn)}), 
and Mark Twain is its \emph{author} (\ms{author(Huckleberry-Finn, Mark-Twain)},
we conclude that Mark Twain must be a \emph{person}.
...
As Mark Twain is not explicitly defined to be a \emph{person}, however, a constraint violation is raised.
