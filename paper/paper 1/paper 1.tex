...
The intention of the paper is not to present any concrete normative suggestion or solution on how constraints for RDF validation should be represented
as there are already five promising languages which can be used to formulate constraints on RDF data. 
...
\emph{DL} can be used to express constraints like (1) \emph{publications} must have at least one \emph{author} which must be a \emph{person} (\ms{Publication $\sqsubseteq$ $\geq$1 author.Person}) and
(2) \emph{books}, \emph{conference proceedings}, and \emph{journal articles} are \emph{publications} (\ms{Book $\sqcup$} \ms{ConferenceArticle $\sqcup$} \ms{JournalArticle $\sqsubseteq$ Publication}). 
...
The effects of reasoning on the computational complexity of constraint types...

The developed expressivity scores enable systems to recommend languages depending on individual use cases and therefore needed constraint types... 
...
(3) we classify RDF constraints according to CWA/UNA dependency, reasoning, and computational complexity (Section \ref{classification-rdf-constraints}).
...
\begin{itemize}
	\item Users should decide which constraints to use for completing data by reasoning 
	and which ones should be considered as constraints about data accuracy and completeness.
	These constraints could be transformed into SPARQL queries and checked over the data once completed using reasoning.
	\item The purpose of this paper is to provide a basis for the development of constraint languages.
	\item Tools, which apply the proposed classification system, should guide users to choose the right language for validation or to make trade-offs around the kinds of validation they need.
	\item To gain a better understanding about the role of reasoning for data
  quality and in order to direct the further development of constraint languages, we investigated ...
	\item We formulate common types of constraints in \emph{Description Logics} and use this formulation to determine their possible complexity when certain reasoning steps are involved. 
  \item At last, we aggregate these findings for the most common constraint languages and show their commonalities and differences regarding constraint expressivity. 
  \item We argue that so far no single best solution exists and that the results presented in this paper are crucial for a proper application of RDF validation.
\end{itemize}
...
%\end{itemize}
For this paper, we identified in total 74 requirements to formulate RDF constraints; each of them corresponding to a constraint type.\tb{ToDO: 74 - 82} 
...
\end{ex}
If we execute reasoning on data containing the triples \ms{publisher(A+Conference} \ms{Proceedings, A+Publisher)} and \ms{rdf:type(A+Publisher, Publisher)}, 
...
Without reasoning, constraints on {\em publications} are not validated as {\em A+ConferenceProceedings} is not part of the class extension of {\em publication}.
...
For the majority of the constraint types, reasoning is not needed to be performed prior to validation. \tb{unclear!}
...
\end{table}\tb{The numbers in parentheses in table 2: what are these
extensions/restrictions?  The subsequent ordering of expressivity
seems to include these values in the ranking: why?}
%\end{center}
...
From Table \ref{tab:expressivity-sores-according-to-expressivity-classes} (with numbers), one important finding is that in terms of expressivity on constraint types, 
...
\tb{ToDo: requirements in brackets}
