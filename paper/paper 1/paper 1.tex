...
Bosch et al.\footnote{available at: \url{https://github.com/boschthomas/rdf-validation-research/paper}} explained each requirement in detail and give at least one example for each of them expressed by different DSCLs.
We added mappings to DL in order to logically underpin each requirement and in order to determine which DL constructs are needed to express each associated constraint.
When we know he DL constructs needed to express a particular constraint, we can determine the needed expressivity and the complexity. 
...
A \equiv \exists <property A> . <range> \\
B \equiv \exists <property A> . <range> \\ 
\end{eqnarray*}
...
The DL constructs \ms{property inclusion}, \ms{complement}, \ms{intersection}, \ms{union}, \ms{existential restriction} are used to express this constraint.  
...
\cite{BoschNolleAcarEckert2015}

\tb{How to publish the requirements evaluation? as technical report?}

\subsection{Classification Dimension Expressivity}

There are no reserved words for expressivity like LOGSPACE for complexity in computational logic.
For the expressivity of DSCLs, we need to define our own reserved words.



\begin{evaluation}
OWL 2 QL reasoning axioms (11) & & & & & & \\
OWL 2 DL reasoning axioms (18) & & & & & & \\
reasoning constraints not supported by OWL 2 (4) & 1 & 0 & 0 & 2 & 2 & 4 \\ 
constraints without reasoning (41) & & & & & \\
\end{evaluation}

The total amount of constraints/axioms is indicated by numbers in round brackets. 
OWL 2 DL reasoning axioms indicate additional reasoning axioms which cannot be expressed by OWL 2 QL. 
OWL 2 QL reasoning axioms are not included in the total amount of OWL 2 DL reasoning axioms.
There are some reasoning constraints which cannot be expressed by OWL 2 such as class-specific property ranges and class-specific reflexive object properties.

%SPARQL is the most (without reasoning) expressive DSCL
%
%we also want to take reasoning capabilities into account
%
%2 expressivity classes: Expressivity 1 (need to name expressivity classes)
%one with reasoning let's say constraints and 1 without
%
%OWL 2 DL is more expressive for the first class
%
%as we have all relevant RDF constraints in our evaluation considered
%we have here the full expressive power
%
%one can use it as functional style like
%Expr(ShEx) $\subset$ Expr(SPIN)
%partial ordering, not necessarily total
%maybe two of such Expr. class are not comparable
%
%so this depends on the use case
%when i want to use reasoning I can use these DSCL when I need to express these constraints
%
%and this can be perfectly integrated in a tool
...
Rather subjective criteria to determine if a DSCL is user-friendliness are:

\begin{itemize}
	\item constraint formulation level: low-level vs. high-level
	\item intuitiveness: not intuitive $<$ less intuitive $<$ intuitive
	\item conciseness: not concise $<$ less concise $<$ concise
\end{itemize}

\begin{user-fiendliness}
level & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & \ding{55} \\
intuitiveness & $\sim$ & $\sim$ & $\sim$ & $\checkmark$ & \ding{55} \\
conciseness & $\sim$ & $\checkmark$ & $\sim$ & $\checkmark$ & \ding{55} \\
\end{user-fiendliness}

\textcolor{red}{examples for each criterion}
...
Within the validation environment, we fully implemented an automatic validation of all OWL 2 DL\footnote{and therefore all OWL 2 QL constructs} constructs. 
