...
RDF validation and reasoning assume different semantics which may lead to different results when applied to particular constraint types. %when particular constraint types are used for validation and for reasoning. 
...
is assumed, i.e., we examine for each constraint type if (1) it depends on the \emph{CWA} and (2) if it depends on the \emph{UNA} (Section \ref{CWA-UNA-dependency}).
