...
\keywords{RDF Validation, RDF Constraint Types, Data Quality, Reasoning, OWL, RDF Validation Requirements, Linked Data, Semantic Web} % NOT required for Proceedings
...
correspond to \emph{OWL 2} axioms and as reasoning and therefore \emph{OWL 2} have different semantics than RDF validation (Section \ref{CWA-UNA-dependency}).
...
(1) Reasoning may resolve violations. If we apply reasoning before validating, the violation is resolved since the implicit triple {\small\ms{Person(Mark-Twain)}} is inferred and therefore made explicitly available. 
...
The \emph{SPIN} engine checks for each resource if it satisfies all constraints, which are associated with classes assigned to the resources, and generates a result RDF graph containing information about all constraint violations.
...
as well as major constraint types representable by \emph{ReSh} and \emph{ShEx}\footref{spin-mappings}.
...
We completely implemented reasoning capabilities for all $\mathcal{R}$ constraint types which are expressible by \emph{OWL 2 QL} and \emph{OWL 2 DL}\footref{spin-mappings}. 
...
We investigated for each constraint type if reasoning may be performed prior to validation in order to enhance data quality
either (1) by resolving violations or (2) by raising valuable violations.
For each constraint type we examined how efficient in terms of runtime validation is performed with and without reasoning in worst case by mapping each constraint type to \emph{DL} (Section \ref{reasoning}).
