...
For data practitioners embracing the world of RDF and Linked Data, the openness and flexibility is a mixed blessing. For them, data validation according to predefined constraints is a  much sought-after feature, particularly as this is taken for granted in the XML world. Several approaches exist to meet this requirement, ranging from using OWL as a constraint language to SPIN, a SPARQL-based way to formulate and check contraints. There are also specific constraint languages like Shape Expressions, Resource Shapes or Description Set Profiles that more or less explicitly address the aforementioned data practitioners.
...
We recently published a technical report\footnote{Available at: \url{https://github.com/boschthomas/RDF-validation-research/paper}} (serving as appendix of this paper) in which we explain each requirement in detail and in which we give examples for each (represented by listed constraint languages).
This technical report also contains mappings to DL to logically underpin each requirement and to determine which DL constructs are needed to express which constraint type\tb{ToDo for Thomas: add reference}.
...
\tb{Erman, can you please verify this sub chapter? Does this make sense?}
...
If we use sub-properties without reasoning and our data contains the triple \ms{editor(JWS14V29, Staab)},
then the triple \ms{creator(JWS14V29, Staab)} has to be stated explicitly.
If this triple is not stated, a constraint violation occurs.
If we use sub-properties with reasoning, the triple (which would cause a constraint violation) is inferred and therefore resolves the possible constraint violation.
...
Without reasoning, the triple \ms{WeavingTheWeb author TimBernersLee} causes a constraint violation, when it is not explicitly stated that \ms{TimBernersLee} is a \ms{Person}. 
When reasoning is performed before validating, the class assignment \ms{TimBernersLee a Person} is inferred which prevents the constraint violation to be raised.
...
\end{ex}
If we execute reasoning on data containing the triples \ms{ISWC-2014-Proceedings publisher Springer} and \ms{Springer a Publisher}, 
...
{\em Context-Specific Exclusive OR of Properties}\footnote{Corresponds to {\em R-11-CONTEXT-SPECIFIC-EXCLUSIVE-OR-OF-PROPERTIES}}.
