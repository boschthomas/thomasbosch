...
and always use constraints that would fit to this scenario. 

basic constraints to achieve high data quality:
\begin{itemize}
	\item Books, conference, and journal articles are publications (subsumption)
	\item Only publications can have {\em author} relationships to persons (property domain)
	\item Publications must have at least 1 author which must be a Person (qualified cardinality restrictions)
	\item Publications must have publishers (existential quantification)
	\item Publications can only have persons as authors (universal quantification)
	\item If a publication is related to a person via the property {\em writtenBy}, this publication is also related to this person via the property {\em author} (sub properties)
  \item Journal of web semantics articles have the editors 'Finin, 'Staab', 'Horrocks', and 'Bernstein' (allowed values)
	\item Identifiers of publications are either ISBN (for books) or ISSN (for periodical publications), but not both (class-specific exclusive or of property groups)
	\item If no keyword is stated for semantic web publications, default keywords are 'Semantic Web' and 'Linked Data' (default values)
	\item ISBN identifiers must conform to specific patterns (Literal Pattern Matching)
\end{itemize}
