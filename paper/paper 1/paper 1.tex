...
\begin{eqnarray*}
%Expr(\mathcal{C}): ... \\
Expr(\overline{\mathcal{C}_R}): DSP \succ OWL2QL \succ ReSh \succ ShEx \succ OWL2DL \succ SPIN \\
Expr(\mathcal{C}_R): DSP \succ OWL2QL \succ ShEx \succ ReSh \succ OWL2DL \succ SPIN
\end{eqnarray*}
...
If, on the other side, only specific requirements in $\overline{\mathcal{C_R}}$ or $\mathcal{C_R}$ are needed for particular use cases, 
the relative expressivity of both constraint classes can be the same for the constraint languages DSP and OWL2-DL, 
even though the overall expressivity of OWL2-DL is much higher for both constraint classes.
