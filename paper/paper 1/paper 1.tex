...
\er{We need a short introduction here.}
...
It contains also mappings to DL to logically underpin each requirement and to determine which DL constructs are needed to express each constraint.
...
\ms{Literal Pattern Matching}\footnote{corresponds to R-44-PATTERN-MATCHING-ON-RDF-LITERALS}, e.g., restrict literals to match given patterns.
Luke's droids can only have the numbers "R2-D2" or "C-3PO".
The universal restriction can be expressed by OWL 2 DL:
...
\textbf{Universal Quantifications}\footnote{corresponds to R-91-UNIVERSAL-QUANTIFICATION-ON-PROPERTIES}
