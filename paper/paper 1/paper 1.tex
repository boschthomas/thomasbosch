...
\er{This section must be rechecked at the end before submission} 
...
\er{Do we really need a Section 3?}
...
When the DL constructs  which is needed to express a particular constraint are known, the required expressivity and the computational complexity to perform reasoning for that constraint can be determined. 
...
The W3C Data Shapes WG defines \ms{constraint} as a component of a schema what needs to be satisfied\footnote{\url{See https://www.w3.org/2014/data-shapes/wiki/Glossary}.}.
...
\ms{Constraints without reasoning} are constraints for which reasoning cannot be done or does not improve in any obvious sense.
...
\ms{LukesDroids $\sqsubseteq$ $\forall$ droidNumber . DroidNumber}.\er{This is no OWL.}
The restriction of the datatype \ms{DroidNumber}, however, cannot be expressed in DL, but OWL 2 DL can be used anyway: \er{DL can naturally express datatypes. All you need to do is to define \emph{DroidNumber} as a concrete domain.}  
...
The first axiom explicitly declares \ms{DroidNumber} to be a datatype. \er{Why start from second and then first? Also better we say "owl axiom" when we talk axiom of owl, to prevent ambiguity.}
...
but not for the triples \ms{Luke hasDroid R2-D2} and \ms{R2-D2 droidNumber "R2-D2"\textasciicircum{}\textasciicircum{}DroidNumber}. \er{Let us divide this last sentence of the paragraph.}

\ms{Constraints with reasoning} are constraints for which reasoning may be performed or is even needed.
\ms{Constraints with reasoning} correspond to axioms in DL - e.g. subsumption, subproperties, cardinality restrictions, existential, and universal restrictions.
...
See that this is not a subclass of constraint anymore. \er{Alright, this whole part was my answer from my email. This was not meant to be put into the paper but rather an explanation for us, but no worries, I will take care of this part tomorrow.}
...
\end{itemize}
\er{Let us better not call these ones as complexity classes, because a complexity class is for instance "PTime". If I understand correctly, this can be called as "validation class" per (computational) complexity. I will help in this tomorrow.}
The complexity class \ms{C-R} encompasses all \ms{constraints with reasoning} for which reasoning can be executed but cannot be expressed by OWL 2.  \er{What/Why can't  be expressed by OWL 2  while being inferred? }
...
\ms{Constraints with reasoning} can be used as axioms to infer new triples out of explicitly stated triples (see section \ref{sec:rdf-validation-requirements-and-reasoning}), but also to express constraints. \er{Also to express constraints?}
...
\ms{constraints with reasoning} cannot only be expressed by OWL 2. \er{This is again the part I don't understand.}
