...
If we express the mentioned constraints in OWL %\er{\tiny \vspace{0.2cm}  But we already did in former paragraph via DL not OWL which would also confuse the reader, we can instead reformulate it here for the first time in OWL or DL,  or in former paragraph in parentheses "in DL:"}
...
\end{enumerate}
 which corresponds to two disjoint subsets of $\mathcal{C}$. \tb{this paragraph is about constraint without reasoning. I would add the whole set of constraints to the list above} \er{Please re-read the paragraph, and a quick continuation, I think it is good now. Also, from that point on, lets refer to `constraints with reasoning` etc. by set names which is better.}$\mathcal{C}_R$ corresponds to set of constraints which can carry out the validation process via reasoning, especially when all knowledge is not explicit.  
Its complement, $\overline{\mathcal{C}_R}$, is the set of constraints for which reasoning cannot be done or does not improve the result in any obvious sense (all the knowledge is already explicit).
...
We evaluated the five most popular constraint languages on fulfilling each requirement to formulate RDF constraints.
...
In this section, we determine the expressivity of constraint languages in terms of classical computer science expressivity classes as well as an expressivity score on reasoning.
As we identified three constraint-specific complexity classes in the previous section, we are now able to determine the performance (i.e. complexity) of constraint languages. 
...
Expressivity scores (on how many of those constraints can be expressed by a particular constraint language) are:
...
\emph{constraints with reasoning} cannot only be expressed by OWL 2 but also by other constraint languages. 
...
\begin{eqnarray*}
%Expr(\mathcal{C}): ... \\
Expr(\overline{\mathcal{C}_R}): DSP \subset OWL2-QL \subset ReSh \subset ShEx \subset OWL2-DL \subset SPIN \\
Expr(\mathcal{C}_R): DSP \subset OWL2-QL \subset ShEx \subset ReSh \subset OWL2-DL \subset SPIN
\end{eqnarray*}
It depends on the individual use case which constraint language to use in order to fulfill related requirements, i.e., to express associated constraints.
...
the relative expressivity of both constraint classes can be the same for the constraint languages DSP and OWL2-DL, 
even though the absolute expressivity of OWL2-DL is much higher for both constraint classes.
...
%\end{itemize}
The next table shows the number of constraints covered by constraint languages and aggregated by expressivity class:
\begin{center}
\begin{evaluation}
...
\begin{eqnarray*}
\emph{Expr(DSP):} DSP \succ SPIN \succ OWL2-DL \succeq ShEx \succ ReSh \succ OWL2-QL \\ 
\emph{Expr(OWL2-DL):} \\
\emph{Expr(OWL2-QL):} \\
\emph{Expr(ReSh):} ReSh \succ SPIN \succ ShEx \succ OWL2-DL \succ DSP \succ OWL2-QL \\
\emph{Expr(ShEx):} \\
\emph{Expr(SPIN):} \\ 
\end{eqnarray*}
...
\tb{do we need this section? do we need a figure?}
