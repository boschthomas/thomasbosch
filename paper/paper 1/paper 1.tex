...
% -----
As part of our earlier work \cite{BoschEckert2014}, we initiated a comprehensive database (publicly available at \url{http://purl.org/net/RDF-validation}) on RDF validation requirements to collect case studies, use cases, and requirements. It is continuously updated and used to evaluate and to compare various existing solutions for RDF constraint formulation and validation. 
...
Yet, none of them is a all-purpose standard, the most popular ones are
...
Their relevance varies with respect to several foremost criteria in the validation process, such as constraint expressivity (what kind of constraints can be expressed?), performance (how efficient in terms of runtime can RDF data be validated?), as well as intuitiveness and conciseness (how easy is it for a user to define constraints?). 
...
{\em Description logics (DL)} is ... \tb{Erman, do you have a very short sentence to describe DL for someone who does not know DL?}. 
...
(2) reasoning experts to raise the awareness for practical applications of reasoning.
...
$\mathcal{C}_{CWA}$ contains constraint types for which it makes a difference if CWA/UNA or OWA/nUNA are assumed.
example...

$\overline{\mathcal{C}_{CWA}}$ includes constraint types for which it does not make any difference if CWA/UNA or OWA/nUNA are assumed.
example...

I would expect that a constraint type can be
...
ex2: If something is a publication, it has at least one author (\ms{Publication $\sqsubseteq$ $\geq$1 author.Person}) ({\em qualified cardinality restriction})

ex3: If someone is a women, it cannot be a man.
ex3: If something is a book, it cannot be a journal article (\ms{Male $\sqcap$ Female $\sqsubseteq$ $\perp$}) ({\em disjoint classes})
