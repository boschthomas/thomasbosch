...
Furthermore, we know that {\em Huckleberry-Finn} is a book  (i.e., \ms{Book(Huckleberry-Finn)}) and \emph{Luke has the weapon laser sword} (\ms{author(Huckleberry-Finn, Mark-Twain)}.  
However,  if a laser sword is not explicitly defined to be a Jedi weapon (e.g., \ms{Jediweapon(LaserSword)}), it causes a constraint violation.
