...
%(3) to globally determine if for all $\mathcal{R}$ constraint types reasoning should be done. 
In addition, we implemented reasoning capabilities for all constraint types for which \emph{OWL 2 QL} and \emph{OWL 2 DL} reasoning may be performed to improve data quality\footref{spin-mappings} (Section \ref{implementation}).
...
wichitge arbeit um ordnung zu schaffen / weitere arbeit an den sprachen / um die weitere arbeit zu vereinfachen stelen wir einen implementierung vor / ann benutzt werden zur weiteren arbeit an constraint languages um die basis zu schaffen wa die impementierung angeht um implementierungsoptionen vergleichen zu können / als erstes vor motivation: arbeit die in verschienden technical reports basiert.. die contribution ist die umfangreiche auswertung der datenebank,k komplette implemntierung vollständig dokumentuert / implementerung als open soure in diesem paper wollen wir die highlights präsentieren / diese arbeit ist die basis um besser über begrifflichkeiten zu reden / sprachen können in sparql umgesetzt werden / comunity abhängige vorlieben für sprachen sparql gute basis zum imepementierung und umsetzung aber on top umfangreichenr sprachen /validierung  constraint languages beschränkt sich nicht nur sondern im umfeld reasoning und validierung gehen hand in hand da bereits beim festlegen der constraints festegen ob reasoning oder nicht interoprable lösung für datenvalidierung eigentlich nur funktioneiren kann wenn man über die einigeung auf einen gemeinsame constraint languge sondern welche semntics zugrundeliegen und reasoning ausgeführt werden kann dass validierung so ausgeführt wird so wie gewollt 
