...
In an OWA setting, in the given knowledge base above, we infer that {\em Huckleberry-Finn} has a title (since {\em Huckleberry-Finn} is a book e.g.,\ms{Book(Huckleberry-Finn)}), but we do not know which exactly, and $\mathcal{K}$ is consistent. \tb{k is consistent does not fit into the text flow}
