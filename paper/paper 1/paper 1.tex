...
Within the \emph{DCMI RDF Application Profiles Task Group} and in cooperation with the \emph{W3C RDF Data Shapes Working Group}, 
we initiated a collaboratively curated database of RDF validation requirements\footref{rdf-validation-database} 
which contains the findings of the working groups based on various case studies provided by data institutions \cite{BoschEckert2014}. 
In this paper, we highlight the key findings from the analysis of the RDF validation requirements. 
Based on our work in these working groups, 
...
Each of these requirements corresponds to a constraint type \cite{BoschNolleAcarEckert2015}.
...
(2) it is needed to rely on the same underlying semantics, and 
...
We mapped constraint types to \emph{DL}
