...
(3) $\overline{\mathcal{R}}$ to denote the 46 constraint types for which reasoning does not improve data quality in any obvious sense as.
...
We consider the \emph{OWL 2} sub-languages \emph{OWL 2 DL} and \emph{OWL 2 QL}, as they differ with regard to expressivity and efficiency in performance (see Section \ref{sec:RDF-validation-requirements-and-reasoning}).
...
Percentage values include these constraint types as well, since it only matters that they are representable by a particular language.
...
enables systems to recommend the right language to formulate constraints depending on individual use cases.
...
Even though, \emph{OWL 2 DL} constraint types are fully implemented,
we cannot use them directly for validating RDF data since \emph{OWL 2} and RDF validation assume different semantics, i.e., validation results depend on underling semantics. 
