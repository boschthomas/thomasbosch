...
For simplicity reasons, we use the terms \emph{constraint types/constraints} instead of \emph{RDF constraint types/RDF constraints} in the remainder of the paper.
...
We recently published a technical report
...
\begin{table}[H]
	\centering
	  %\caption{Constraint Type Specific Expressivity}
		\caption{Overview on the Constraint Type Specific Expressivity of Constraint Languages (Percentage Values)}
	  \scriptsize
		\begin{tabular}{l|c|c|c}
      %\textbf{Expressivity Class} & $Expr(\overline{\mathcal{CT}_R})$ (42) & $Expr(\mathcal{CT}_R)$ (32) \\		
			\textbf{} & $\mathcal{CT}$ (81) & $\overline{\mathcal{R}}$ (46) & $\mathcal{R}$ (35) \\	
      \hline
			\emph{DSP} & 17.3 & 13.0 & 22.9 \\
			\emph{OWL2-DL} & 67.9 & 45.7 & 97.1 \\ 
			\emph{OWL2-QL} & 24.7 & 19.6 & 31.4 \\
			\emph{ReSh} & 25.9 & 15.2 & 40.0 \\
			\emph{ShEx} & 29.6 & 26.1 & 34.3 \\
			\emph{SPIN} & 100.0 & 100.0 & 100.0
		\end{tabular}
	\label{tab:constraint-type-specific-expressivity-percentage-values}
\end{table}
Tables \ref{tab:constraint-type-specific-expressivity-absolute-values} and \ref{tab:constraint-type-specific-expressivity-percentage-values} 
give an overview on the constraint type specific expressivity of the five most common constraint languages.
For \emph{OWL 2}, we differentiate between the sub-languages \emph{OWL 2 DL} and \emph{OWL 2 QL}, as they differ with regard to expressivity and efficiency in performance (see Section \ref{sec:RDF-validation-requirements-and-reasoning}).
Table \ref{tab:constraint-type-specific-expressivity-absolute-values} contains the absolute numbers of supported constraint types by constraint language, 
Table \ref{tab:constraint-type-specific-expressivity-percentage-values} includes appropriate percentage values.
...
enables validation environments to recommend the right language to formulate constraints depending on individual use cases.
...
Furthermore, the fact that all $\mathcal{R}$ constraint types and $\overline{\mathcal{R}}$ constraint types are representable by \emph{SPIN} (if we ignore the constraint type \emph{recursive queries} which is partly implemented in \emph{SPIN}) 
...
are expressible with \emph{OWL 2 DL}.
Even though, $\mathcal{R}$ constraint types corresponding to \emph{OWL 2 DL} axioms are fully implemented,
we cannot use them directly for validating RDF data since \emph{OWL 2} and RDF validation assume different semantics, i.e., validation results depend on different underling semantics. 
...
Consider the following \emph{DL} knowledge base $\mathcal{K}$ which is a collection of formal statements  which corresponds to \emph{facts} or  \emph{what is known} explicitly:
{\scriptsize
\begin{center}
\begin{DL} 
$\mathcal{K}=\{$ \\ 
...
\emph{OWL 2} reasoning requires the \emph{OWA} with the \emph{nUNA} whereas RDF validation has its origin in the XML world and is therefore classically based on the \emph{CWA} and the \emph{UNA}.
While \emph{DLs/OWL} do not assume \emph{UNA}, they have the construct \emph{owl:sameAs} to express when two names are the same or not.
...
A SPIN construct template contains a SPARQL CONSTRUCT query which generates constraint violation triples (\emph{spin:Constraint Violation}) indicating the subject (\emph{spin:violationRoot}) and the properties (\emph{spin:violationPath}) causing constraint violations, and the reason why constraint violations have been raised (\emph{rdfs:label}).
...
As a consequence, the \emph{SPIN} construct template generates a constraint violation triple.

As \emph{property ranges} is an $\mathcal{R}$ constraint type 
for which reasoning may be performed prior to validation to enhance data quality by resolving violations.
Therefore, validation environments should enable users to decide if reasoning should be executed for the \emph{property ranges} constraint type. 
If a users decides to use reasoning for the \emph{property ranges} constraint type, 
the triple {\small\ms{rdf:type(Doyle, Person)}}, whose absence caused the violation, is inferred before the validation is executed which resolves the violation. 
...
(3) to globally decide if for all $\mathcal{R}$ constraint types reasoning should be done. 
All resources, for which reasoning should be performed prior to validation, are assigned to the class \emph{Reasoning}.
The class \emph{Reasoning} is associated with these resources either manually (1) or automatically (2+3) in terms of a pre-reasoning step.
...
within our \emph{SPIN} validation environment in form of \emph{SPARQL CONSTRUCT queries}.
