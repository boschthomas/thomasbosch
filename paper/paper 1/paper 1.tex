...
Within the \emph{DCMI Task Group}, we initiated a collaboratively curated database of RDF validation requirements which contains the findings of the working groups based on various case studies provided by data institutions \cite{BoschEckert2014}. It is publicly available and open for further contributions\footnote{Online at \url{http://purl.org/net/rdf-validation}\label{rdf-validation-database}}.
...
we defined by today 81 RDF constraint types \cite{BoschNolleAcarEckert2015}
...
that validation results are different depending on 
...
According to our experience, however, the topic data validation is yet far too often reduced to suitable constraint languages
which may be related to the obvious and yet inaccurate comparison with XML as basis technology to represent data.
With this paper, we want to make clear that it depends on more than the used constraint language when validating RDF data and when developing appropriate systems.
...
(3) It must be determined if reasoning should be involved in the validation process. Necessary reasoning steps have to be predefined to allow the correct interpretation of the constraints, for instance, if constraints are defined on super-classes to avoid redundancy.
...
(2) it may cause valuable violations, and 
(3) it solves the redundancy problem.
Users should be enabled to select which constraint types to use for completing data by reasoning before data is validated 
...
As part of future work we extend the \emph{RDF Validator} to provide a list of languages for which the expressivity is enough to represent constraint types
...
with the lowest complexity class which is acceptable for the user.
