...
If something is a publication, e.g., it has at least one author (\ms{Publica} \ms{tion $\sqsubseteq$ $\geq$1 author.Person}).
...
\end{ex}
If we execute reasoning on data containing the triples \ms{publisher(ISWC-2014-} \ms{Proceedings, Springer)} and \ms{rdf:type(Springer, Publisher)}, 
...
[ a owl:Restriction ;
  ...
  RDFs:subClassOf Publication ] .
\end{ex}
When performing reasoning, the triples \ms{The-Lord-Of-The-Rings author} \ms{JRR-} \ms{Tolkien} and \ms{The-Lord-Of-The-Rings a Publication} infer that \ms{JRR-Tolkien} is a person.
