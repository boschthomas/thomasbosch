...
\textcolor{red}{introduction of OWL 2 QL and DL}
...
\textcolor{red}{The subsequent steps are needed to get an idea about the performance, i.e., the computational complexity, to validate constraint types:}
\begin{enumerate}
...
we examine if reasoning can be performed prior to validation to enhance data quality.
	\item \textcolor{red}{Since reasoning makes difference  for some validation types, we note them with the computational complexity  of  the language that they are expressed in.}
...
Without reasoning, the triple {\small\ms{author(Alices-Adventures-In-Wonderland, Lewis-Carroll)}} leads to a violation
...
To have a further evaluation on the latter and to refer easily, we divide $\mathcal{CT}_R$ into two classes (not disjoint). 
Overall, we have the following. Since, \emph{OWL 2 DL} is more expressive than \emph{OWL 2 QL},  $\mathcal{C}_R ^{\mathcal{QL}} \subseteq \mathcal{C}_R ^{\mathcal{DL}}$ follows. 
\begin{enumerate}\itemsep0.5em
	\item $\overline{\mathcal{CT}_R}$: validation by query answering without reasoning
	\item $\mathcal{CT}_R ^{\mathcal{QL}}$: validation by query answering with \emph{OWL 2 QL} reasoning
	\item $\mathcal{CT}_R ^{\mathcal{DL}}$: validation by query answering with \emph{OWL 2 DL} reasoning
\end{enumerate}
Recall that, in this work we limit $\mathcal{CT}_R$ to the constraint types which can be expressed by \emph{OWL 2 DL} and still requires reasoning 
(i.e., $\mathcal{CT}_R$ = $\mathcal{CT}_R ^{\mathcal{DL}}$). This is due the fact that, \emph{OWL 2 DL} is still quite expressive\footnote{It could alternatively be set to a more expressive language (e.g., \emph{OWL 2 Full}). Such treatment can be done in future work, if it is understood that we require that.}. 
Some of these constraint types can still be expressed by constraint languages which is the focus of the next section.
...
Table \ref{tab:constraint-type-specific-complexity-classes} gives an overview over the complexity for RDF validation when reasoning is performed and when reasoning is not performed.
\begin{table}
\centering
...
What is considered by RDF validation with or without reasoning corresponds to performing \emph{SPARQL} queries.  
It is known that performing \emph{SPARQL} queries is in \textsc{Pspace}-Complete \cite{Perez2009}. 
Since \emph{OWL 2} profiles are based on the \textit{DL-Lite} family and query answering in \emph{OWL 2 QL} (which is based on \textit{DL-Lit}$e_R$ is in \textsc{LogSpace} (or rather in \emph{AC$^0$} ) \cite{Calvanese2007}, so is the constraint validation by queries with  reasoning. 
Furthermore, TBox reasoning is performed in \emph{OWL 2 QL} is in \textsc{Ptime} \cite{Calvanese2007}, 
hence complete query rewriting as well as combined complexity (reasoning and than querying) is in \textsc{Ptime} \cite{Artale2009,Calvanese2007}.  
...
We do not include \emph{OWL 2 Full} 
...
\textcolor{red}{reviews:}
\begin{itemize}
	\item \textcolor{red}{The term “expressivity scores” might be useful but a stronger argument is needed.}
\end{itemize}
...
In evaluating the expressivity,  we introduce the intuitive notion of \emph{expressivity scores} rather than classical expressivity. The main motivation in that is, the list of constraint types is an updated list, and almost none (except SPIN) of the aforementioned languages contains one another.
...
While we evaluate expressivity scores, we use two sets of constraint types as before: $C_R$ and $\overline{C_R}$. 
The main motivation in that is, that one may want to perform reasoning before actually validating the data.  For the sake of simplicity, we abbreviate \emph{expressing $C_R$} as $Expr(C_R)$, and \emph{expressing $\overline{C_R}$} as $Expr(\overline{C_R})$.
...
some constraint types in $\mathcal{CT}_R$ are not only expressible by OWL 2  but also by other constraint languages. 
...
Numbers in paranthesis in the other columns indicate the number of constraint types which may be expressed by that particular constraint language - either by limitations, workarounds, or extensions.  
...
On the other hand, if only specific requirements in $\overline{\mathcal{C_R}}$ or $\mathcal{C_R}$ are needed, 
...
all constraint languages can be fully covered by SPIN if we ignore one last constraint type (i.e., the recursive queries which is partly implemented in SPIN). 
Except for SPIN, all supported constraint types are already implemented by particular constraint languages.
