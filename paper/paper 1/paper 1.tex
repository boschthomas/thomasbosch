...
However, as Mark Twain is not explicitly defined to be a person (e.g., \ms{Person(Mark-Twain)}), it causes a constraint violation.
If we express the mentioned constraints in OWL
and apply a reasoner before the proper validation is performed, we will not get a constraint violation as implicit triples such as \emph{Mark Twain is a person} (i.e., \ms{Person(Mark-Twain)}) will be inferred. 
