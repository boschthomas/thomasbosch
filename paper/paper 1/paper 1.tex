...
is assumed, i.e., we examine for each constraint type (1) if the constraint type depends on the CWA and (2) if the constraint type depends on the UNA (Section \ref{CWA-UNA-dependency}).
...
for which DL provides the foundational basis and
which offers knowledge representation and reasoning services. 
Validation, however, is not the primary purpose of its design which has lead to claims that OWL 2 cannot be used for validation. 
...
OWL 2 DL was standardized as a DL-like formalism with high expressivity, yet maintains decidability for main reasoning tasks.
...
As OWL 2 profiles are based on the \textit{DL-Lite} family,
...
\begin{ex}
Publication owl:disjointUnionOf ( C1 C2 ) . 
C1 rdfs:subClassOf [ a owl:Restriction ;
    owl:onProperty isbn ;
    owl:someValuesFrom xsd:string ] .
C2 rdfs:subClassOf [ a owl:Restriction ;
    owl:onProperty issn ;
    owl:someValuesFrom xsd:string ] .
\end{ex}
...
\emph{Books} on the topics \emph{Computer Science} and \emph{Librarianship}, e.g., should only have \emph{ComputerScience} and \emph{Librarianship} as \emph{subjects}.
...
In an OWA setting, this constraint does not cause a violation, even if there is no explicitly defined \emph{title}, since there must be a \emph{title} for this \emph{book} which we may not know ($\mathcal{K}$ is consistent).
...
In the example, the CWA yields to a violation since there is no explicitly defined \emph{title} for the \emph{book} {\em Hamlet}. 
...
Although, \emph{DLs/OWL 2} do not assume UNA, they have the constructs \emph{owl:sameAs} and \emph{owl:differentFrom} to state that two names are the same or different.
...
We classify constraint types according to the dependency on the CWA and on the UNA which leads to four sets of constraint types:
...
{\em Minimum qualified cardinality restrictions} (\emph{R-75}) is a $\mathcal{CWA}$ constraint type, i.e., depends on the CWA.
\emph{Publications}, e.g., must have at least one \emph{author} {\small\ms{(Publication $\sqsubseteq$ $\geq$1 author.Person).}}
In a CWA setting, a \emph{publication} without an explicitly stated author violates the constraint, 
whereas, with OWA semantics, 
a \emph{publication} without an explicitly stated \emph{author} does not raise a violation 
as the constraint entails that there must be an \emph{author} which we may not know.
...
For \emph{Functional properties} (\emph{R-57/65}), it makes a difference with regard to validation results if UNA or nUNA is assumed.
As the property \emph{title} is functional {\small\ms{(funct (title)),}} a \emph{book} can have at most one distinct \emph{title}.
UNA causes a clash
...
For nUNA, however, reasoning concludes that the \emph{title} {\em The-Adventures-of-Huckleberry-Finn} must be the same as the \emph{title} {\em Die-Abenteuer-des-Huckleberry-Finn} 
...
if it is dependent on the CWA and on the UNA (for a detailed analysis we refer to the appendix of this paper \cite{BoschNolleAcarEckert2015}). 
...
validation environment\footnote{Online demo available at: \url{http://purl.org/net/rdfval-demo}, source code available at: \url{https://github.com/boschthomas/rdf-validator}\label{rdf-validator}} to validate RDF data according to constraints expressed by arbitrary constraint languages by mapping them to SPIN\footnote{SPIN mappings available at: \url{https://github.com/boschthomas/rdf-validation/tree/master/SPIN}\label{spin-mappings}} \cite{BoschEckert2014-2}.
...
We provide implementations to validate constraints of all constraint types expressible by OWL 2 QL, OWL 2 DL, and DSP 
as well as constraints of major constraint types representable by ReSh and ShEx\footref{spin-mappings}.
By means of a \emph{property ranges} (\emph{R-28, R-35}) constraint it can be restricted that \emph{author} relations can only point to \emph{persons} (DL: {\small\ms{$\top$ $\sqsubseteq$ $\forall$ author.Person,}} OWL 2 DL: {\small\ms{author rdfs:range Person.).}}
...
A SPIN construct template contains a \emph{SPARQL CONSTRUCT} query generating constraint violation triples which indicate the subject, the properties, and the constraint causing the violations and the reason why violations have been raised.
...
A SPIN construct template creates violation triples if all triple patterns within the \emph{SPARQL WHERE clause} match.
...
Therefore, validation environments should enable users to decide if reasoning on constraints of the \emph{property ranges} constraint type should be executed before RDF data is validated. 
If a user decides to use reasoning on the \emph{property ranges} constraint type, 
...
There is one SPIN rule for each $\mathcal{R}$ constraint type, so for the $\mathcal{R}$ constraint type \emph{property ranges}:
...
A SPIN rule contains a \emph{SPARQL CONSTRUCT} query which generates triples 
if all triple patterns within the \emph{SPARQL WHERE clause} match.
In case \emph{Doyle} is not defined to be a \emph{person}, all triple patterns match and the triple {\small\ms{rdf:type(Doyle, Person)}} is created. As a consequence, the violation on \emph{Doyle} is not raised. 
...
The first method implies a materialized knowledge base, where the original knowledge base is extended by all assertions that can be inferred. State-of-the-art DL or \emph{OWL} reasoners following this approach are \emph{FaCT++} \cite{tsarkov2006fact++}, \emph{Pellet} \cite{sirin2007pellet},  \emph{RacerPro} \cite{haarslev2001racer}, or \emph{HermiT} \cite{horrocks2012hermit}. 
...
whether validation is based on the \emph{closed-world assumption (CWA)} or on the \emph{open-world assumption (OWA)} and 
whether the \emph{unique name assumption (UNA)} or the \emph{non-unique name assumption (nUNA)} is underlying.
...
By mapping constraint types to \emph{description logics}, we were able to determine their computational complexity (Section \ref{reasoning}).
...
Using these findings, we determined for the five most common constraint languages which constraint types they enable to express \cite{BoschNolleAcarEckert2015}.
...
As part of future work we extend the \emph{RDF Validator} to provide a list of languages for which the expressivity is sufficient to represent constraint types
