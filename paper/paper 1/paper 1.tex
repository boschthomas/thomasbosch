...
In this paper, we investigate the expressivity, the complexity, and the user-friendliness of each constraint language and constraint.
We take OWL 2 reasoning into account as a pre-validation step which leads to higher complexity.
In this case, users of validation tools could define which axioms they want to use for their inference rules.
...
When stating needed axioms and constraints (without reasoning capabilities), 
the validator is able determine the expressivity class and thus can recommend which constraint language to use in order to express the wished axioms/constraints. 
Although, there is a partial ordering of constraint languages of particular expressivity classes, 
validators may not recommend the constraint language with the highest expressivity of that specific expressivity class.
The most expressive constraint language may not be as good evaluated regarding user-friendliness as another constraint language, which is less expressive but can also express the needed axioms/constraints.
