...
RDF documents follow the structure of RDFS/OWL ontologies which can therefore be reused for validation. In particular, most of the constraint languages except OWL, have a major shortcoming of redundancy. \tb{explain why this reduces redundancy}
Consider the following DL knowledge base $\mathcal{K}$\footnote{A  knowledge base is a collection of formal statements which corresponds to \emph{facts} or  \emph{what is known}.}:
\tb{does this new example suit our needs?}
\begin{center}
\begin{DL} 
$\mathcal{K}=\{$ & \ms{Book $\sqsubseteq \exists$ title.$\top$}, \ms{Book $\sqsubseteq \forall$ author.Person }\\
 &(\ms{funct (title)}),\\
 &\ms{Book(Huckleberry-Finn)},\\
...
As each book must have a title (\ms{Book $\sqsubseteq \exists$ title.$\top$}}) and as {\em Huckleberry-Finn} is a book (\ms{Book(Huckleberry-Finn)}),
the book {\em Huckleberry-Finn} must have at least one title.
...
Hence, classical constraint languages are based on CWA where constraints need to be satisfied only by named individuals which yields in return to a constraint violation. This would be a shortcoming of OWA in RDF validation. \tb{needs further explanation}
We can further observe that there is a clash caused by the functional property $\ms{title}$ and the UNA. \tb{explain why? surely not obvious to everybody}
However, in OWA there is no UNA by default, therefore the reasoning will conclude that {\em The-Adventures-of-Huckleberry-Finn} is the same individual as {\em Die-Abenteuer-des-Huckleberry-Finn} (\ms{The-Adventures-of-Huckleberry-Finn $\equiv$ Die-Abenteuer-des-Huckleberry-Finn}). This is not a problem if we add the UNA to the OWA, like in the \textit{DL-Lite} family \cite{Calvanese2007,Artale2009} which is the basis for OWL 2 QL. 
...
\tb{Erman, in this table we need more spacing between the rows}
%\begin{center}
\begin{table}
\centering
%\begin{tabular}{| c | c |}
...
\end{table}
%\end{center}
\tb{Erman, can you reduce the space between the table caption and the following text please? and expand the space between the table and the caption?}
...
For each of them, ee presented representative constraint types to investigate the effects of reasoning to the validation process.
