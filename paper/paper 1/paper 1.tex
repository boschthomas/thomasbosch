...
The database, which is publicly available and open for further contributions\footnote{Online at \url{http://purl.org/net/rdf-validation}\label{rdf-validation-database}}, connects requirements to use cases, case studies, and solutions.
...
(5) We provide open source validation and reasoning implementations of constraint types to be used as a basis to drive the further development of constraint languages (Section \ref{implementation}).
...
In general, \emph{RDF} documents follow the syntactic structure and the semantics of \emph{RDFS}/\emph{OWL 2} ontologies which could therefore not only be used for reasoning but also for validation. 
...
which may be related to the obvious but yet inaccurate comparison with XML as basis technology to represent data.
...
Necessary reasoning steps have to be predefined to allow the correct interpretation of the constraints, for instance, if constraints are defined on super-classes to avoid redundancy.
...
either (1) by resolving violations or (2) by raising valuable violations.
For 43.2\% of the constraint types, reasoning may be performed before actually validating to improve data quality.
...
We provide open source validation and reasoning implementations of constraint types to be used as a basis to drive the further development of constraint languages (Section \ref{implementation}).
