...
\tb{ToDo: we need table captions, to refer to tables, and tables centered}
...
\begin{itemize}
	...
	\item \ms{Expr-R}: reasoning constraints not supported by OWL
...
\begin{itemize}
	\item \ms{constraint formulation level}: low-level (\ding{55}) vs. high-level ($\checkmark$)
	\item \ms{intuitiveness}: not intuitive (\ding{55}) $<$ less intuitive ($\sim$) $<$ intuitive ($\checkmark$)
	\item \ms{conciseness}: not concise (\ding{55}) $<$ less concise ($\sim$) $<$ concise ($\checkmark$)
	\item \ms{adoption}: less adopted (\ding{55}) $<$ well adopted ($\checkmark$)
\end{itemize}
...
adoption & \ding{55} & $\checkmark$ & \ding{55} & \ding{55} & $\checkmark$ \\
...
DSP, ReSh, as well as OWL 2 are not that intuitive than ShEx, but more intuitive to humans than SPIN\footnote{see the examples of the requirement \ms{allowed values} in the paper of Bosch et al. \cite{BoschNolleAcarEckert2015}}.
...
\tb{adapt venn diagramm: expressivity classes, DSCL ordering, user-fiendliness}
...
\tb{ToDo for Andy: validation in terms of inconsistency detection}
