...
SPARQL is generally seen as the method of choice to validate data according to certain constraints, although it is not ideal for their formulation. 
In contrast, more intuitive and concise constraint languages are comparatively easy to understand, can be placed on top of SPARQL 
but lack an implementation to validate RDF data.

SPARQL and SPIN are powerful and widely used for constraint formulation and validation \cite{Fuerber2010}, but constraints formulated as SPARQL queries are not as understandable as one wishes them to be.

SPARQL is a very good basis for the implementation of constraint languages
...
We provide these open source validation and reasoning implementations of constraint types to use them as a basis to drive the further development of constraint languages (Section \ref{implementation}).
...
When constraints are defined one has to determine if reasoning should be executed.
