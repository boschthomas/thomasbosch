...
\emph{Property ranges} (\emph{R-28, R-35}) constraints restrict ranges of properties:
\emph{author} relations can only point to \emph{persons}.

\begin{ex}
owl2spin:ObjectPropertyRange a spin:ConstructTemplate ;
...
For all resources for which reasoning should be performed prior to validation (resources of the class \emph{Reasoning}) the \emph{SPIN} rule is executed. 
...
\begin{ex}
    ...
    :Peter 
        a owl:Thing, owl2spin:Reasoning ;
        :hasDog :Brian .
    :Brian 
        a owl:Thing, owl2spin:Reasoning . 
    # --> :Brian a :Dog . 
