...
\emph{Description Set Profiles (DSP)}, \emph{Resource Shapes (ReSh)}, \emph{Shape Expressions (ShEx)}, and the \emph{Web Ontology Language} \emph{(OWL)}. Despite the fact that OWL is arguably not a constraint language, it is widely used in practice as such under the closed-world and unique naming assumptions.
...
We further see that OWL 2 is currently the most expressive high-level constraint language, at least according to the pure number of contraint types supported. This does not preclude that other constraint languages are better suited for certain applications, either because they support some types that are not supported by OWL or because the constraint representation is mroe appealing to the data practitioners -- producers as well as consumers who again might have different needs and preferences.
...
%%% DL introduction, not needed here...
% -This constraint can either be represented generically (\emph{generic constraint}) by \emph{description logics (DL)}: {\small\ms{Publication $\sqsubseteq$ $\geq$1 author.Person}} or
...
% \begin{enumerate}
%  \item We develop a framework to describe constraints of any constraint type in a generic way (Section \ref{sec:vocabulary}).
%  \item We demonstrate the application of this framework by mapping different constraint language to a unified intermediate representation that is not only mapped to SPIN, but also enables transformations to semantically equivalent constraints in other high-level constraint languages (Section \ref{sec:transformations}).
%  \item We separate the implementation of the constraint type in SPIN from its expression in a high-level constraint language which makes the implementation reusable and allows a consistent validation across constraint languages (Section \ref{sec:validation}).
% \end{enumerate}
% 
...
% \section{How to Provide the Validation of RDF Constraints Out-Of-The-Box}
