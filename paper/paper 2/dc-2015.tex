...
The DL statements demonstrate that the complex constraint is composed of many other complex constraints (e.g., \emph{minimum and maximum qualified cardinality restrictions}) and simple constraints (e.g., constraints on sets). As \emph{exact (un)qualified cardinality restrictions (=n)} and \emph{exclusive or (XOR)} are frequently used complex constraints, we propose to simplify them as simple constraints in terms of syntactic sugar. As a consequence, the \emph{context-specific exclusive or of property groups} complex constraint is represented as a generic constraint by means of the RDF-CV more intuitively and concisely (see Table \ref{tab:simplified-complex-constraints}).
...
the complex constraint can be formulated as a simple constraint which looks like the following in concrete RDF turtle syntax:
