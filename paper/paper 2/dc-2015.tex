...
The creation of such mappings is in many cases not straight-forward and requires profound knowledge of SPARQL, as the following example demonstrates. \tb{diese beipsiel zeigt nur wie constraints desslben typs mit unterschiedlichen CL ausgedrückt wird! sollen wir hier noch ein codebeipsiel eines konkreten mappings dieses CT für DSP zeigen?}
...
In this paper, we build on the experience gained from mapping several constraint languages to SPIN and from the analysis of the identified constraint types \tb{was haben wir analysiert? meinst du die constraint type specific expressivity of constraint languages?}to create an intermediate layer, a framework that is able to describe the mechanics of all constraint types and that can be used to map high-level languages more easily. \tb{sollten wir hier nicht alle contributions nennen? ich denke im abstact stehen sie alle}
