...
The \emph{constraining element} is an intuitive term which indicates the actual type of constraint. For the majority of the constraint types, there is exactly one constraining element, for instance \emph{property domain (R-25, R-26)} restricts domains of properties and there is only one constraining element with exactly the same identifier \emph{property domain}. Complex constraints, however, need several constraining elements to be expressed, for instance \emph{language tag cardinality (R-48, R-49)}\ke{ich hab das etwas umgestellt, bin mir aber nicht sicher, ob r-48/49 complex sind. Ggf. bitte anpassen, eventuell den satz oder absatz auch rausziehen vor "simple constraints", wenn da erst mal geklärt wird, was das ist.} is used to restrict data properties to have a minimum, maximum, or exact number of relationships to literals with selected language tags. Thus, three constraining elements are needed to express each possible constraint of that constraint type. This example also illustrates that the granularity of the constraint types varies and certainly often is debatable. Keep in mind that they correspond to requirements as identified by the working groups. The constraining elements, as in this example, are closer to atomic elements of constraints.
...
Book & isbn$^{-}$ & - & - & functional property & - \\
		\end{tabular}
\end{table}
\ke{eins der isbn- in der tabelle ist doch falsch, oder? inverse von isbn- ist doch isbn...}
