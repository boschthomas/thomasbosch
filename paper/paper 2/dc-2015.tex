...
%
OWL 2 is based on the {\em open-world assumption} (OWA), i.e., a statement cannot be inferred to be false if it cannot be proved to be true  which fits its primary design purpose to represent knowledge on the World Wide Web. 
...
many RDF validation scenarios require the {\em closed-world assumption} (CWA), i.e., a statement is inferred to be false if it cannot be proved to be true.
...
The CWA yields to a violation since there is no explicitly defined title for the book {\em Hamlet}. 
%
...
If we define the property title to be functional {\small\ms{(funct (title)),}} a book can have at most one distinct title.
UNA causes a clash
...
This is important 
as for 56.8\% of the constraint types validation results differ if the CWA or the OWA 
