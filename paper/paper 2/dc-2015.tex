...
our purpose is not to invent yet another constraint language.
But as non of the current constraint languages does support all constraint types, 
we developed a lightweight vocabulary to intermediately represent constraints of any constraint type in a generic way.
Even though, this vocabulary is not considered to be a constraint language,
constraints represented by the proposed vocabulary are intuitively understandable 
as their structure is based on RDF terminology. 
As we developed a vocabulary to represent constraint of any constraint type, 
we are therefore able to provide a basic terminology and classification system for RDF constraints 
to lay the ground for discussions on RDF validation.

When using the developed vocabulary to intermediately represent constraints,
depending on the individual type of constraint particular constraining elements can be applied.
...
we determined intuitive constraining elements in natural language like \emph{conditional properties}, \emph{allowed values}, and \emph{language tag minimum cardinality}.
Constraints of the constraint type \emph{required properties}, e.g., can be expressed by the existential quantifier $\exists$ or more intuitively using the constraining element \emph{required properties}.
...
(2) represented generically by the proposed vocabulary. 
...
%to determine which \emph{DL} constructs are needed to express them \cite{BoschNolleAcarEckert2015} (Section \ref{sec:vocabulary}).
