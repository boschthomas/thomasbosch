...
%XML Schema is the standard language to formulate constraints to validate XML documents.
%For RDF, however, there are several constraint languages - but not a stand-alone standard one meeting the majority of RDF validation use cases. 
...
% Truth be told, we do may two things, blabla on and to blabla, however for the flow of the text we dont need to overkill the text. In abstract and introduction, we just give general idea so at such points we better focus on flow.  
...
We evaluated which constraint types the five most common constraint languages enable to express.
...
The fact that \emph{SPIN} supports all and that \emph{OWL 2} covers 2/3 of all constraint types
...
In contrast, high-level constraint languages, which may be placed on top of \emph{SPARQL} and \emph{SPIN}, are comparatively easy to understand and constraints can be formulated more concisely. 

-----

For the formulation of constraints, it is important that the constraint language is concise and intuitive and that the declarative constraint language is translated to an implementation language like SPARQL in order to execute constraint validation automatically.

-----

There exists no single best solution considered as high-level intuitive constraint language which enables to express constraints easily and concisely and which is able to meet all requirements raised by data practitioners.
...
This constraint can either be represented generically (\emph{generic constraints}) by \emph{description logics (DL)} {\small\ms{(Publication $\sqsubseteq$ $\geq$1 author.Person)}} or specifically (\emph{specific constraints}) by multiple domain-specific constraint languages:
...
and since the book \emph{The-Lord-Of-The-Rings} is a \emph{publication}, 
...
we developed a vocabulary to describe constraints of any RDF constraint type generically (Section \ref{sec:vocabulary}).
(3) We show how to enhance the interoperability of constraints expressed by different constraint languages by using the proposed vocabulary as an intermediate generic representation which enables transformations between semantically equivalent constraints of the same constraint type (Section \ref{sec:transformations}).
(4) We explain how arbitrary constraint languages may reuse validation implementations which are provided once for each constraint type (Section \ref{sec:validation}).
...
(2) to resolve misunderstandings in the communication of RDF data producers and consumers, and
...
Without an intermediate generic representation, we would have to define for each constraint type one mapping between each pair of \emph{specific constraints} expressed by \emph{n} languages
- that are \emph{\( {n \choose 2} \)} mappings (\emph{\( {10 \choose 2} \)} = 45 mappings).
With an intermediate generic representation, on the other side, we only need to define for each constraint type \emph{n} mappings 
...
When language designers extend newly developed or existing constraint languages 
in order to allow to formulate constraints of not yet supported constraint types,
...
We enable to offer validation implementations for any constraint type whose \emph{specific constraints} may be expressed by any language out-of-the-box 
by providing a \emph{SPIN} mapping
%generic validation implementations once 
for each constraint type\footnote{RDF-CV to SPIN online available at: \url{https://github.com/boschthomas/RDF-CV-2-SPIN}}  whose constraints are represented generically (for further details see \cite{BoschEckert2015-2}).
%(2) by transforming \emph{specific constraints} into the appropriate \emph{generic constraint}.  
All what language designers have to do is to specify for each constraint type one mapping from the \emph{specific constraint} expressed by a particular language to the corresponding \emph{generic constraint}.
...
i.e., that validation is performed independently of the used language.
This is not the case, however, for existing languages since they are implemented independently form each other.
...
As we use multiple languages to express constraints of constraint types in the appendix of this paper,
...
%Although, \emph{OWL 2} does not assume \emph{UNA}, \emph{OWL 2} has the constructs \emph{owl:sameAs} and \emph{owl:differentFrom} to state that two names are the same or different.
...
%\emph{OWL 2} reasoning is based on the {\em open-world assumption (OWA)}, i.e., a statement cannot be inferred to be false if it cannot be proved to be true. 
...
%%Classical constraint languages are based on the \emph{CWA} where constraints need to be satisfied only by named individuals which yields a constraint violation.
%This ambiguity in semantics is one of the main reasons why \emph{OWL 2} has not been adopted as a standard constraint language for RDF validation in the past. 
...
%In case we want to use \emph{OWL 2} as a constraint language, we adopt the same semantics (CWA) that is for RDF validation in general. 
...
%(1) We mapped constraint types to \emph{DL} to logically underpin them and 
%(2) we described the constraint types' intended behavior in terms of validation results depending on \emph{CWA} and \emph{UNA}.
...
%we show for each constraint type that a constraint expressed by a particular language leads to the same answer as the corresponding \emph{DL} query.
...
%If a constraint is represented by \emph{ShEx} and \emph{OWL 2}, 
...
%In general, validation has to be implemented once for each \emph{generic constraint type} which is not expressible in \emph{DL}. 
%\begin{gcotable}
...
In this section, we present current approaches for constraint formulation and validation which have been the most discussed in the mentioned workshops and WGs.
These approaches differ in 2 dimensions: (1) the used constraint language and (2) if they offer validation systems.

OWL, Resource Shapes (ReSh), Shape Expressions (ShEx), Description Set Profiles (DSPs), SPARQL, and SPIN are the most promising and applied constraint languages.
Stardog ICV, Pellet ICV, and SPIN use OWL 2 constructs to formulate constraints.
SPIN\footnote{http://spinrdf.org} provides a vocabulary to represent SPARQL queries as RDF triples
and uses SPARQL to specify inference rules and logical constraints \cite{Fuerber2010}.
The Pellet Integrity Constraint Validator (ICV)\footnote{http://clarkparsia.com/pellet/icv/} is a proof-of-concept extension for the OWL reasoner Pellet. 
Stardog ICV\footnote{http://docs.stardog.com/icv/icv-specification.html} validates RDF data stored in a Stardog RDF database. 
ReSh\footnote{http://www.w3.org/Submission/shapes/} defines its own RDF vocabulary Open Services for Lifecycle Collaboration (OSLC) to define constraints \cite{Ryman2013}. 
ShEx\footnote{http://www.w3.org/2013/ShEx/Definition} also specifies a new constraint language whose syntax and semantics are similar to regular expressions.
DCMI RDF Application Profile (AP)\footnote{http://dublincore.org/documents/singapore-framework/} and Bibframe\footnote{http://bibframe.org/} are approaches to specify profiles for application-specific purposes. 
DCMI RDF-AP uses DSP\footnote{http://dublincore.org/documents/dc-dsp/} as generic constraint language which is also intuitive for non-experts.
The Bibframe constraint language has a strong overlap with DSP.
Kontokostas et al. define 17 data quality integrity constraints represented as SPARQL query templates called Data Quality Test Patterns (DQTP) \cite{Kontokostas2014}. 
Schemarama\footnote{http://swordfish.rdfweb.org/discovery/2001/01/schemarama/} is based on the Squish RDF language instead of SPARQL. 
For XML, Schematron\footnote{http://www.schematron.com/} is an ISO standard for validation and quality control of XML documents based on XPath and XSLT. 
XML Schema\footnote{http://www.w3.org/TR/xmlschema-1/} is the primary technology for specifying and constraining the structure of XML documents.
