...
constraints which are \emph{expressible in DL} (constraints of this set are instantiated from 70\% of the overall 81 constraint types) and which are \emph{not expressible in DL} (30\%). 
...
\emph{Simple constraints} (2/3 of the constraint types) denotes the set of atomic constraints. 
...
in order to determine which \emph{DL} constructs are needed to express constraint types \cite{BoschNolleAcarEckert2015}.
Prerequisites to develop a vocabulary to describe constraints of any constraint type in a generic way are 
(1) to specify the underlying semantics for validation and thus for the vocabulary and (2) to define a basic terminology and classification system for RDF constraints which lay the ground for discussions on validation \ref{sec:vocabulary}).
