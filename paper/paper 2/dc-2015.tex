...
the book must have at least one \emph{author} relationship to a \emph{person}.
The constraint is violated either (1) if
\emph{The-Lord-Of-The-Rings} does not have any \emph{author} relationship, 
or (2) if the book has an \emph{author} which is not a \emph{person}
{\small\ms{(author(The-Lord-Of-The-Rings, Tolkien)},} {\small\ms{rdf:type(Tolkien, Hobbit)),}} 
or (3) if the book has an \emph{author} whose class is unknown
...
semantically equivalent \emph{minimum qualified cardinality restrictions} may be represented by multiple languages - each of them having different syntax and semantics.
This causes confusion,
weakens the common understanding between several parties about the meaning of particular constraints, and
raises the question on how to ensure high data quality. 
Therefore, when choosing a language, say \ms{$\alpha$}, to express a constraint of an arbitrary constraint type, 
it should be possible to transform the constraint into a semantically equivalent constraint expressed by another constraint language \ms{$\beta$} (see Section \ref{sec:transformations}). 
...
since automatically performed constraint transformations avoid the necessity to understand several languages.
...
The next step, however, is to extend the \emph{RDF Validator} to validate RDF data conforming to constraints of each of the 81 constraint types. 
 
Validation on semantically equivalent constraints, expressed by different constraint languages, 
should lead to identical validation results, 
i.e., validation should be performed independently from the used language (see Section \ref{sec:validation}).

We are able to offer already implemented validation for constraints of constraint types expressed by arbitrary languages out-of-the-box, 
(1) if we implement the validation of the corresponding generically represented constraint types once and 
(2) if we transform semantically equivalent specifically expressed constraints into generically expressed constraints.  
...
(3) We show how to transform constraints, expressed by any constraint language \ms{$\alpha$}, into generically expressed constraints and into constraints represented by any other constraint language \ms{$\beta$} and discuss why these transformations are useful (Section \ref{sec:transformations}).
...
Nearly 15\% of the \emph{complex constraints}, however, can be formulated as \emph{simple constraints} when using them in terms of syntactic sugar. 
\textbf{\emph{Context}:}
...
\tb{explain how typical constraints are expressed by the most common constraint languages}
