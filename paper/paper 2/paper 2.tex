...
\tb{split introdution and motivation}
\tb{feedback from Erman: the major problem is semantics. OWL basically work in different semantics than others. And their semantics are formal. Closed world vs. Open World is also a part of this.}
...
There is not one language meeting the majority of requirements to formulate RDF constraints.
Even though there are more than 10 languages with different syntaxes and semantics which can be used to express RDF constraints (e.g. existential and universal quantification, cardinality restrictions, and exclusive or of properties), the language selection depends on the individual use case.
The five most promising ones on being the standard are
\emph{Description Set Profiles (DSP)}\footnote{\url{http://dublincore.org/documents/2008/03/31/dc-dsp/}},
\emph{Resource Shapes (ReSh)}\footnote{\url{http://www.w3.org/Submission/shapes/}}, 
\emph{Shape Expressions (ShEx)}\footnote{\url{http://www.w3.org/Submission/shex-primer/}},
the \emph{SPARQL Inferencing Notation (SPIN)}\footnote{\url{http://spinrdf.org/}}, 
and the \emph{Web Ontology Language (OWL 2)}\footnote{\url{http://www.w3.org/TR/owl2-syntax/}}.
%\section{Ideas}
...
The idea behind this paper is to express any constraint type (corresponding to a requirement to formulate RDF constraints) in a generic way using a lightweight vocabulary (consisting of only a few terms) (section \ref{sec:vocabulary}).
...
\end{ex}
\tb{ToDO: delete the slash in front of the dollar sign from the output of the code fragment above (in the stated pattern)?}
...
OWL provides a variety of others, namely \emph{role transitivity}, \emph{symmetry}, \emph{asymmetry}, \emph{reflexivity} and \emph{irreflexivity}. 
These are sometimes considered as basic axiom types in DL as well, using some suggestive notation such as
\ms{Trans(ancestorOf)} to express that the role \emph{ancestorOf} is transitive. \tb{transitive property example from the library domain}
However, such axioms are just syntactic sugar; 
all role characteristics can be expressed using the basic features of DL.
The \emph{Irreflexive Object Properties} constraint type\footnote{Corresponds to \emph{R-60-IRREFLEXIVE-OBJECT-PROPERTIES}}, e.g., 
...
The irreflexivity constraint \ms{$\top$ $\sqsubseteq$ $\neg$ $\exists authorOf.Self$}
states that individuals cannot be authors of themselves.
