...
We defined the formal generic semantics of the \emph{RDF-CV} (see appendix of this paper)
as we mapped constraint types to DL and as we described the intended \emph{CWA} and \emph{UNA} behavior of constraint types.
In order to specify transformations between specific constraints,
first we have to ensure identical semantics.
Constraint languages assume identical semantics, 
if for each constraint the following condition holds:
If a given constraint is represented by two different languages, 
the validation results must be exactly the same,
i.e., whenever it is violated in one language it is also violated in the other language.
In order to show that languages have the same semantics,
we expressed constraint types in different languages.
