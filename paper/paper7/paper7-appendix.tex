...
\begin{itemize}
	\item \textbf{{\em DISCO-C-XX:}} for each \emph{rdfs:Resource x}, there can be at most one distinct \emph{rdfs:Resource y} such that \emph{x} is connected by \emph{adms:identifier} to \emph{y}.
	
\begin{DL}
$(\textsf{funct } \textsf{identifier})$
\end{DL}
\end{itemize}
...
If dcterms:date and one of its sub-properties dcterms:created or dcterms:issued are present, check that the value in dcterms:date is not redundant with dcterms:created or dcterms:issued for ingestion.
