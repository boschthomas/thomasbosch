...
The following DL statements holds for each pair of \emph{Disco} properties (not having the same domain and range classes) ($variable \sqsubseteq \neg question$).
\end{itemize}
...
The following DL statements holds for each pair of \emph{Data Cube} properties (not having the same domain and range classes) ($dataSet \sqsubseteq \neg structure$).
\end{itemize}
...
the \emph{intersection}\footnote{\ms{R-15-CONJUNCTION-OF-CLASS-EXPRESSIONS}, \ms{R-16-CONJUNCTION-OF-DATA-RANGES}} (also called \emph{conjunction})
...
\begin{itemize}
  ...
  \item \textbf{{\em DATA-CUBE-C-EXISTENTIAL-QUANTIFICATIONS-05:}}
	...
	Every \emph{qb:SliceKey} must be associated with (\emph{qb:sliceKey}) a \emph{qb:DataStructureDefinition}. 
\end{itemize}
...
\begin{itemize}
	...
	Every \emph{qb:Observation} has (\emph{qb:dataSet}) exactly one associated \emph{qb:DataSet}. 
	\item \textbf{{\em DATA-CUBE-C-EXACT-QUALIFIED-CARDINALITY-RESTRICTIONS-02:}}
	Unique DSD (\emph{IC-2} \cite{CyganiakReynolds2014}) -  
	Every \emph{qb:DataSet} has (\emph{qb:structure}) exactly one associated \emph{qb:DataStructureDefinition}. 
\end{itemize}
...
\subsection{\underline{Hierarchies}}
...
\begin{itemize}
	\item \textbf{{\em DATA-CUBE-C-HIERARCHIES-01:}}
	...
	\item \textbf{{\em DATA-CUBE-C-HIERARCHIES-02:}}
