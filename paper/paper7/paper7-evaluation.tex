...
\begin{table}[H]
    ...
    \\       \textbf{Criteria (Counts)}
           & \textbf{\rot{Person-Level Metadata (Disco)}}
           & \textbf{\rot{Aggregated Metadata (Data Cube)}}
					 & \textbf{\rot{Thesauri (SKOS)}}
					 & \textbf{\rot{Rectangular Data (PHDD)}}
					 & \textbf{\rot{Statistical Classifications (XKOS)}}
					 & \textbf{\rot{Total}}
    \\ \midrule
		\emph{Validated Triples} & 9,673,055 & 3,775,983,610 & 477,737,281 \\
		\emph{Validated Data Sets} & 1,526 & 9,990 & 4,178 \\
		\hline
		\emph{Constraint Violations} & \textcolor{red}{XXXXX} \\
		\emph{Constraint Violations ($\mathcal{SL}_{0}$)} & \textcolor{red}{XXXXX} \\
		\emph{Constraint Violations ($\mathcal{SL}_{1}$)} & 473,574 $\equiv$ \textcolor{red}{XXXXX}\% \\
    \emph{Constraint Violations ($\mathcal{SL}_{2}$)} & \textcolor{red}{XXXXX} \\
				\hline
		\emph{Constraint Types} & 52 (15\textbar 37) \\
		\emph{Constraint Types} ($\mathcal{C}_{C}$) & 30 $\equiv$ 57.7 \% \\
		\emph{Constraint Types} ($\mathcal{C}_{T}$) & 22 $\equiv$ 42.3 \% \\
		\hline
		\emph{Constraints} & 142 (77\textbar 65) \\
		\emph{Constraints} ($\mathcal{C}_{C}$) & 72 $\equiv$ 50.7 \% \\
		\emph{Constraints} ($\mathcal{C}_{T}$) & 70 $\equiv$ 49.3 \% \\
		\emph{Constraints ($\mathcal{SL}_{0}$)} & 75 $\equiv$ 52.8\% (44\textbar 31)\\
		\emph{Constraints ($\mathcal{SL}_{1}$)} & 9 $\equiv$ 6.3\% (8\textbar 1)\\
		\emph{Constraints ($\mathcal{SL}_{2}$)} & 58 $\equiv$ 40.8\% (25\textbar 33)\\
    \bottomrule
    \end{tabular}
    \caption{Evaluation}
...
\begin{table}
	\centering
      ...
      \textbf{Symbol} & \textbf{Description} \\		
			\textbf{\emph{X}} & Validation Successful \\
			\hline
      ...
      \ding{55} & Very Poor Performance/Scaling \\
			...
			\textbf{\emph{(X)}} & The validation of \emph{X} data sets could not be finished \\
			           & due to SPARQL endpoints' technical restrictions (e.g. the defined timeout). \\
			\hline
			\textbf{\emph{\textsuperscript{*}}} & severity level $\mathcal{SL}_{0}$ (informational) \\
			\hline
			\textbf{\emph{\textsuperscript{**}}} & severity level $\mathcal{SL}_{1}$ (warning) \\
			\hline
			\textbf{\emph{\textsuperscript{***}}} & severity level $\mathcal{SL}_{2}$ (error) \\
		\end{tabular}
	\caption{Legend}
	\label{tab:legend}
\end{table}
...
\section{Person-Level Metadata (Disco)}

\subsection{Evaluation Results}

Legend: total (implemented\textbar not yet implemented)
...
\begin{table}[H]
    ...
    \emph{Constraint Violations ($\mathcal{SL}_{2}$)} & \textcolor{red}{XXXXX} \\
		...
		\emph{Constraint Types} ($\mathcal{C}_{T}$) & 22 $\equiv$ 42.3 \% \\
		\hline
		\emph{Constraint Types (Most Constraints)} & 1. Existential Quantifications: 46 $\equiv$ 32.4\% (46\textbar 0) \\
		                                            & 2. Data Model Consistency: 7 (1\textbar 6) \\
																								& 3. Aggregation: 7 (0\textbar 7) \\
		\emph{Constraint Type (Most Constraints ($\mathcal{SL}_{2}$))} & Existential Quantifications: 9 (9\textbar 0) \\
		\hline
		\emph{Constraints} & 142 (77\textbar 65) \\
		\emph{Constraints} ($\mathcal{C}_{C}$) & 72 $\equiv$ 50.7 \% \\
		\emph{Constraints} ($\mathcal{C}_{T}$) & 70 $\equiv$ 49.3 \% \\
...
\section{Aggregated Metadata (Data Cube)}
...
\begin{table}[H]
	\centering
      ...
      \hline
		...
		\emph{Constraint Violations ($\mathcal{SL}_{0}$)} & 0 $\equiv$ 0\% \\
		\emph{Constraint Violations ($\mathcal{SL}_{1}$)} & 45,520,613 $\equiv$ 99.75\% \\
    \emph{Constraint Violations ($\mathcal{SL}_{2}$)} & 115,233 $\equiv$ 0.25\% \\
		\hline
		...
		\emph{Constraint Types} ($\mathcal{C}_{C}$) & 5 $\equiv$ 25 \% \\
		\emph{Constraint Types} ($\mathcal{C}_{T}$) & 15 $\equiv$ 75 \% \\
		\hline
		\emph{Constraint Types (Most Constraints)} & 1. Data Model Consistency: 11 $\equiv$ 31.4\% (10\textbar 1) \\
		                                           & 2. Existential Quantifications: 4 $\equiv$ 11.4\% (4\textbar 0) \\
		\emph{Constraint Type (Most Constraints ($\mathcal{SL}_{2}$))} & Data Model Consistency: 8 $\equiv$ 22.9\% (7\textbar 1) \\
		\hline
		\emph{Constraints} & 35 (20\textbar 15) \\
		\emph{Constraints} ($\mathcal{C}_{C}$) & 16 (12\textbar 4) \\
		\emph{Constraints} ($\mathcal{C}_{T}$) & 19 (8\textbar 11) \\
		\emph{Constraints ($\mathcal{SL}_{0}$)} & 4 $\equiv$ 11.4\% (0\textbar 4) \\
		\emph{Constraints ($\mathcal{SL}_{1}$)} & 3 $\equiv$ 8.6\% (3\textbar 0) \\
		\emph{Constraints ($\mathcal{SL}_{2}$)} & 28 $\equiv$ 80\% (17\textbar 11) \\
		\end{tabular}
...
\begin{table}[H]
	\centering
      ...
      \textbf{Evaluation Criteria} & \textbf{Counts} \\		
      \hline
		\emph{Validated Triples} & \textcolor{red}{XXXXX} \\
		\emph{Validated Data Sets} & \textcolor{red}{XXXXX} \\
		\hline
		\emph{Constraint Violations} & \textcolor{red}{XXXXX} \\
		\emph{Constraint Violations ($\mathcal{SL}_{0}$)} & \textcolor{red}{XXXXX} \\
		\emph{Constraint Violations ($\mathcal{SL}_{1}$)} & \textcolor{red}{XXXXX} $\equiv$ \textcolor{red}{XXXXX}\% \\
    \emph{Constraint Violations ($\mathcal{SL}_{2}$)} & \textcolor{red}{XXXXX} \\
		\hline
		...
		\emph{Constraint Types} ($\mathcal{C}_{C}$) & 5 $\equiv$ 35.7\% \\
		\emph{Constraint Types} ($\mathcal{C}_{T}$) & 9 $\equiv$ 64.3\% \\
		\hline
		\emph{Constraint Types (Most Constraints)} & 1. Structure: 10 $\equiv$ XX\% (8\textbar 2) \\
		                                           & 2. Labeling and Documentation: 6 $\equiv$ XX\% (5\textbar 1) \\
																							 & 3. Language Tag Cardinality: 4 $\equiv$ XX\% (4\textbar 0) \\
		\emph{Constraint Type (Most Constraints ($\mathcal{SL}_{2}$))} & Structure: 1 (0\textbar 1) \\
		\hline
		\emph{Constraints} & 35 (17\textbar 18) \\
		\emph{Constraints} ($\mathcal{C}_{C}$) & 21 (13\textbar 8) \\
		\emph{Constraints} ($\mathcal{C}_{T}$) & 14 (4\textbar 10) \\
		\emph{Constraints ($\mathcal{SL}_{0}$)} & 21 $\equiv$ 60\% (12\textbar 9) \\
		\emph{Constraints ($\mathcal{SL}_{1}$)} & 5 $\equiv$ 14.3\% (5\textbar 0) \\
		\emph{Constraints ($\mathcal{SL}_{2}$)} & 9 $\equiv$ 25.7\% (0\textbar 9) \\
		\end{tabular}
...
\section{Statistical Classifications (XKOS)}
...
\begin{table}[H]
    ...
    \emph{NAF} & \emph{Nomenclature d'activités française}\tablefootnote{\url{http://rdf.insee.fr/codes/index.html}} \\
		...
		\emph{ISIC} & \emph{}\tablefootnote{\url{}} \\
		\emph{ISCO} & \emph{}\tablefootnote{\url{}} \\
		\end{tabular}
...
\begin{itemize}
	\item \emph{Nomenclature d'activités française (NAF)}: French classification expressed in XKOS. the French refinement of the NACE, because it has explanatory notes.
	\item \emph{Nomenclature des Professions et Catégories Socioprofessionnelles (PCS)}: French classification expressed in XKOS.
	\item \emph{Nomenclature des catégories juridiques (CJ)}: French classification expressed in XKOS.
	\item \emph{ISIC}: has explanatory notes too. 
	\item \emph{ISCO}
\end{itemize}
