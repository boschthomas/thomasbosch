...
In this paper, we evaluated the metadata and data quality of large real world aggregated (\emph{QB}), person-level (\emph{Disco}), thesauri (\emph{SKOS}), rectangular (\emph{PHDD}), and statistical classification (\emph{XKOS}) data sets by means of RDF constraints assigned to multiple constraint types.
We validated more than 4.2 billion triples and 15 thousand data sets
using the \emph{RDF Validator} which is available at \url{http://purl.org/net/rdfval-demo}.
