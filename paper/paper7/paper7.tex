...
\textbf{Variables.}
Each variable should have a variable representation (\emph{R-86}; $\mathcal{SL}_{1}$), which is either an (un)ordered code list or a union of XML Schema datatypes.
In case of a code list, associated categories should be stated (\emph{R-71}; $\mathcal{SL}_{0}$).
As the variable \emph{Education at pre-school} has an associated code list, the constraint is not violated.
...
The variable \emph{Education at pre-school} is associated with the theoretical concept \emph{education}. 
The default severity level of this constraint is \emph{warning} and not \emph{error}, as in most cases research can be continued without an associated theoretical concept of a given variable.
...
Restricting data property ranges [$V_{min}$,$V_{max}$] within the context of given classes is a very common requirement (\emph{R-45: literal ranges}) 
...
In \emph{Missy}, we provide comprehensive Linked Data services\footnote{\textcolor{red}{\url{www.gesis.org/en/missy/linked-data}}} like a \textcolor{red}{triple store} and diverse RDF exports of person-level metadata conforming to the \emph{Disco} vocabulary in multiple concrete syntaxes (e.g. turtle). 
