...
This technical report also contains mappings to description logics to logically underpin each requirement and to determine which description logics constructs are needed to express each constraint type \cite{BoschNolleAcarEckert2015}.
...
The continuum of \emph{severity levels} ranges from informational via warning and error to fatal error. 
Although we provide default \emph{severity levels} for each constraint, users should be able to specify \emph{severity levels} of constraints they need to validate within the context of their individual use cases.
Therefore, use case specific \emph{severity levels} can be defined for constraints.
...
\textbf{Variable Comparisons\footnote{Constraint types: \emph{DISCO-C-COMPARISON-VARIABLES-XX}}.}
One very common research question is to compare variables of multiple surveys or countries.
...
Thus, the \emph{severity level} of the first three constraints is stronger than the \emph{severity level} of the next constraints.
