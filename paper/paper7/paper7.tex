...
The database connects requirements to use cases, case studies and implementations and forms the basis of this paper. To gain a better understanding about the role of certain requirements for data quality and in order to direct the further development of constraint languages, we collected constraints for commonly used vocabularies in the \emph{SBE} domain, either from the vocabularies themselves or from domain and data experts. All in all, this lead to TODO constraints on TODO vocabularies\tb{246 constraints on six vocabularies}. We let the experts classify the constraints according to the severity if they are violated. Furthermore, we classified the type of each constraint (corresponding to a requirement) based on its complexity ranging from types commonly found in vocabulary specifications (e.g., domain and range)\tb{and cardinality restrictions}, over types that are simply stated using common constraint languages (e.g., cardinality restrictions\tb{nein, nimm hier language tag cardinalities / cardinalty restrictions können formal in vokabularen spezifiert werden}) to complex types that involve complex data structures or need more sophisticated languages to be easily expressible (e.g., constraints on network topologies\tb{vielleicht besser: constraint on graph-based structures}).
...
To ensure high quality, (meta)data must satisfy certain criteria - specified in terms of RDF constraints.\ke{verwirrend. wie gesagt, können wir nicht bei RDF data bleiben? was anderes behandelt ihr doch eh nicht und qb enthält doch keine metadaten.} \tb{ich wollte nu beschreiben, dass data provider verantwortlich dafür sind, dass ihre daten festgelegten regeln / bedingungen / constraints entsprechen / wenn von mehreren data providern geharvestet wird, ist das noch eine zusätzliche dimension, können wir aber auch weglassen}
...
\tb{sollen wir die beiden appendixe diese paper hier nochmals erwähnen?}
...
\footnote{The second appendix of this paper describing the evaluation in detail is available at: \url{http://arxiv.org/abs/1504.04478} \cite{BoschZapilkoWackerowEckert2015-2}.}
...
(1) The percentage of severe violations is very low, compared to about 2/3 of warning violations and 1/3 of informational violations, which implies that proper constraint languages can significantly improve the data quality beyond fundamental requirements.
(2) A significant amount of 47\% of the violations refer to complex constraints that are not easily, concisely, and intuitively expressible by existing constraint languages which confirms the necessity to provide suitable constraint languages.
(3) As a significant amount of 54\% of the constraints are either simple or complex constraints,
the further development of constraint languages should concentrate on expressing simple and especially complex constraints which up to now in most cases can only be expressed by plain SPARQL. 
