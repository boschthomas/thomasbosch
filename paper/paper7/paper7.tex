...
We recently published a technical report\footnote{\textcolor{red}{Available at: \url{http://arxiv.org/abs/XXXXX}}} (serving as first appendix of this paper) in which we describe constraints to validate metadata on person-level, aggregated data, and thesauri.
...
\subsubsection{Aggregated Metadata and Data}
...
\subsubsection{Person-Level Metadata and Data}. 
...
If the abstract and an external description are absent, 
...
\emph{Existential quantifications} (\emph{R-86}) enforce that instances of given classes must have some property relations to individuals of certain types.
...
If there is no metadata (especially case and variable quantities) on data files including the actual data of data sets ($\mathcal{SL}_{1}$), the description of the data sets and the underlying study is not sufficient.
...
Some constraints ensure the integrity of the data according to intended data model semantics (\emph{data model consistency}).
Codes (\emph{skos:Concept}), e.g, are ordered and therefore have fixed positions in an ordered collection (\emph{skos:OrderedCollection}) within variable representations.
In order to check the correctness of relative frequencies' calculations, the cumulative percentage (\emph{disco:cumulativePercentage}) of the current code must exactly be the cumulative percentage of the previous code (\emph{disco:cumulativePercentage})
...
Types of summary statistics like minimum, maximum, and mode are maintained within a controlled vocabulary.  
Thus, summary statistics can only have \emph{disco:summaryStatisticType} relationships to \emph{skos:Concept}s which must be members of the controlled vocabulary \emph{ddicv:SummaryStatisticType}, a \emph{skos:ConceptScheme} (\emph{R-91: universal quantification}; $\mathcal{SL}_{2}$):
\begin{center}
\begin{DL}
...
\textbf{Coverage.}
Information about the temporal (\emph{dcterms:temporal}), the spatial (\emph{dcterms:spatial}), and the topical coverage (\emph{dcterms:subject}) of series, studies, data sets, and data files (\emph{R-86: existential quantification}; $\mathcal{SL}_{1}$) is of interest when performing particular queries 
	(e.g. to search for all data sets of given years (temporal coverage) in which data is collected in given countries (spatial coverage) about given topics (topical coverage)).
Depending on property datatypes,
two different literal values have
a specific ordering with respect to an operator (e.g. \textless, \textless=, \textgreater, \textgreater=, =, !=) (\emph{R-43: literal value comparison}).
Start dates (\emph{disco:startDate}), e.g., must be before (\emph{\textless}) end dates (\emph{disco:endDate}) ($\mathcal{SL}_{2}$).
It must be made sure that all literal values are valid for their datatypes (\emph{R-223: value is valid for datatype}; $\mathcal{SL}_{2}$).
It has to be ensured, e.g., that all date values (e.g. {\em disco:startDate}, {\em disco:endDate}, {\em dcterms:date}) are really of the datatype \emph{xsd:date}, or that \emph{xsd:nonNegativeInteger} values (e.g. \emph{disco:frequency}) are not negative.
...
In \emph{Missy}, we provide comprehensive Linked Data services\footnote{\textcolor{red}{\url{http://www.gesis.org/missy/eu/general-information/linked-data/}}} like a triple store and diverse RDF exports of person-level metadata conforming to the \emph{Disco} vocabulary in multiple concrete syntaxes (e.g. turtle). 
...
The \emph{RDF Validator} also validates RDF data conforming to SPARQL constraints to ensure correct syntax and semantics of vocabularies such as \emph{Disco, Data Cube, PHDD, DCAT, SKOS, and XKOS}\footnote{\textcolor{red}{SPIN mappings: \url{https://github.com/boschthomas/rdf-validation/SPIN}}}.
The SPIN engine checks for each resource if it satisfies all constraints (associated with its assigned classes) and generates a result RDF graph containing information about all constraint violations.
We define one SPIN construct template for each constraint type (corresponding to a requirement) and vocabulary-specific constraint\footnote{For details about the \emph{RDF Validator} see \cite{BoschEckert2014-2}}.
...
Among others data sets, we validated 
...
We recently published a technical report\footnote{\textcolor{red}{Available at: \url{http://arxiv.org/abs/XXXXX}}} (serving as second appendix of this paper) 
in which we describe the evaluation in detail. 
As we validated 9,990 aggregated data sets, we published the validation results of all aggregated data sets in form of one document per SPARQL endpoint\footnote{\url{https://github.com/boschthomas/rdf-validation/tree/master/evaluation/data-sets/data-cube}}.
Although accessible within the \emph{RDF Validator}, we provide all implemented constraints\footnote{\textcolor{red}{\url{https://github.com/boschthomas/rdf-validation/tree/master/constraints}}} in form of SPARQL CONSTRUCT queries.
The correctness of all evaluated constraints, i.e., the gold standard, has been proved by SBE domain experts.

We defined 142 (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) constraints assigned to 52 (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) distinct constraint types and implemented 77 (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS})  of them to actually validate data sets.  
49.3\% (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) of the defined constraints are $\mathcal{C}_{T}$ constraints, and 
42.3\% (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) of the constraint types are $\mathcal{C}_{T}$ constraint types.
52.8\% (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) of the defined constraints are associated with the default severity level $\mathcal{SL}_{0}$, 6.3\% (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) are $\mathcal{SL}_{1}$, and 
40.8\% (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) are $\mathcal{SL}_{2}$ constraints.
\emph{R-86: Existential quantifications} (32.4\%, \emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) are the constraint types with the most defined constraints.
We got \textcolor{red}{XX} (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) / \textcolor{red}{XX} (\emph{In Total}) constraint violations. 
\textcolor{red}{XX} (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) / \textcolor{red}{XX} (\emph{In Total}) constraint violations caused by violating $\mathcal{SL}_{0}$ constraints, 
473,574 (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) / \textcolor{red}{XX} (\emph{In Total}) by $\mathcal{SL}_{1}$ constraints, and 
\textcolor{red}{XX} (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) / \textcolor{red}{XX} (\emph{In Total}) by $\mathcal{SL}_{2}$ constraints.
\emph{DISCO-C-LABELING-AND-DOCUMENTATION-06} (547,916) and \emph{DISCO-C-COMPARISON-VARIABLES-02} (547,916) (\emph{Disco}) / \textcolor{red}{XX} (\emph{QB}) / \textcolor{red}{XX} (\emph{SKOS}) are the constraints responsible for the most constraint violations.
...
As part of future work, we will identify constraints on rectangular data and statistical classifications and classify them.
