...
Constraints are instantiated from constraint types in order to validate both metadata and data represented by any vocabulary\tb{vocabulary-independent}. 
...
We show how data in rectangular format and metadata on record-unit data sets (i.e., data about individuals, households, businesses) or more specifically person-level data sets, aggregated data sets, thesauri, and statistical classifications are represented in RDF and how therefore reused vocabularies are interrelated.\tb{record-unit or more specific person-level data sets}
...
\tb{DCAT und XKOS nur im future work}
...
\begin{enumerate}
	\item $\mathcal{CT}_{C}$: \emph{Content-Driven Constraint Types}\tb{complex constraints / zahlen interpretierbar / in einem bestimten vokabulare werden viele basic constraints verletzt / daten haben bessere qualität vokabular / wichtigkeit constraint languages, dass complexe constraints beschrieben}
	\item $\mathcal{CT}_{T}$: \emph{Technology-Driven Constraint Types}\tb{basic constraints / welche constraints können automatisch abgeleitet werden / bestimmte constraints werden in den vokabularen definiert bzw sehr leicht zu implementieren wäre / studie ohne daten macht keinen sinn --> cardinality restriction / alle constraints die mit rdf + rdfs + owl definiert werden können / vokabukare werden heutzutage mit rdf / rdfs / owl beschrieben / constraints die typischerweise hinterlegt werden / constraints die amn als vokabular}
\end{enumerate}

\tb{große datenbestände untersuchen um ein besseres verständnis zu bekommen / liste von constraints / jedes einzelne constraint für jeded einzelne vokabular severity level / warum severity was bringt das} 
...
\emph{SBE} experts \tb{todo / wieviele constraints welche klasse} determined the default \textbf{\emph{severity level}} (corresponds to requirement \emph{R-158}) for each constraint to indicate how serious the violation of the constraint is.
...
%\textbf{Constraints Derived From Vocabularies.}
\textbf{Vocabulary.}
One should not invent new or use deprecated terms (e.g. \emph{disco:containsVariable}) of vocabularies (\emph{vocabulary}).
...
\emph{xkos:belongsTo} relationships can only point to \emph{skos:Concepts} (\ms{$\top$ $\sqsubseteq$ $\forall$ belongsTo.Concept}).
...
A \emph{phdd:TableStructure}, e.g., has (\emph{phdd:column}) at least one \emph{phdd:Column} (\ms{TableStructure $\sqsubseteq$ $\geq$1 column.Column}),
a \emph{disco:Variable} has at most one \emph{disco:concept} relationship to a theoretical concept (\emph{skos:Concept}) (\ms{Variable $\sqsubseteq$ $\leq$1 concept.Concept}), and a \emph{qb:DataSet} is structured by (\emph{qb:structure}) exactly one \emph{qb:DataStructureDefinition} (\ms{DataSet $\sqsubseteq$ $\geq$1 structure.DataStructureDefinition $\sqcap$ $\leq$1 structure.DataStructureDefinition}).
...
(\emph{R-9: disjoint properties}), i.e., no individual \emph{x} can be connected to an individual/literal \emph{y} by disjoint properties like \emph{phdd:isStructuredBy} and \emph{phdd:column} ($isStructuredBy \sqsubseteq \neg column$).
...
With \emph{subsumption} (\emph{R-100}), one can state that \emph{xkos:ClassificationLevel} is a sub-class of \emph{skos:Collection}, i.e., each \emph{xkos:ClassificationLevel} must also be part of the \emph{skos:Collection} class extension (\ms{ClassificationLevel $\sqsubseteq$ Collection}).
With \emph{sub properties} (\emph{R-54, R-64}), one can state that \emph{disco:fundedBy} is a sub-property of \emph{dcterms:contributor} - i.e., if a study is funded by an organization, then this organization contributed to this study (\ms{fundedBy $\sqsubseteq$ contributor}).
...
We refer to the technical reports\footnotemark[\ref{technical-report-1}] \footnotemark[\ref{technical-report-2}] to get details about constraints on and the evaluation of \emph{XKOS} and \emph{PHDD} data sets.\tb{bezug nehmen auf severity levels / technology-driven werden oft eingehalten? / }
...
(1) \emph{Content-Driven Validation} $\mathcal{C}_{C}$ contains the set of constraints ensuring that the data is consistent with the intended syntax, semantics, and integrity of data models (section \ref{content-driven-validation}).
...
We exhaustively evaluated the metadata quality of large real world aggregated (\emph{QB}), person-level (\emph{Disco}), and thesauri (\emph{SKOS}) data sets by means of 212 $\mathcal{C}_{C}$ and $\mathcal{C}_{T}$ constraints of the majority of the constraint types.
