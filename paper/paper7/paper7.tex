...
Half of the overall 213 $\mathcal{CT}$ constraints are $\mathcal{CT}_{B}$ constraints (almost 40\% $\mathcal{C}_B ^{\mathcal{R}}$).
...
\end{hyp} 
These days, vocabulary and simple constraints can be easily, concisely, and intuitively expressed by either modeling languages (e.g., RDF, RDFS, OWL 2) or constraint languages (e.g., ShEx, ReSh, and SPIN). 
This is not the case, however, for complex constraints which still have to be expressed by plain SPARQL. 
There is a huge motivation to extend existing constraint languages or to develop new constraint languages which enable to express complex constraints
As a consequence, being able to express complex constraints more easily, concisely, and intuitively leads to significant improvements in data quality.
