...
Each study must have an abstract, i.e., the abstract (\emph{disco:purpose}) should have a minimum length (\emph{xsd:minLength}) of 0.
...
If the abstract of a study is missing, a title has to be stated.

\textbf{Variables.}
Each variable should have a variable representation (\emph{R-86: existential quantification}), which is either an (un)ordered code list or a union of XML Schema datatypes.
...
then the property {\em disco:isValid} has to be stated indicating if the code is valid (\emph{true}) or missing (\emph{false}) (\emph{R-71: conditional properties}).
%\footref{R-71-CONDITIONAL-PROPERTIES}
