...
Eurostat\footnote{\url{http://ec.europa.eu/eurostat}} provides publicly available European aggregated data.\tb{aggregated (meta)data ?}
...
The present data collection refers to data on formal childcare and other types of care by the \tb{aggregated?}variables \emph{year}, \emph{duration} (0 hours, 1 - 29 h/week; 30 h/week)\tb{HPW ?}, \emph{age} of the child (0-2 years; 3 to admission age for compulsory school; admission age for compulsory school to 12) and \emph{country}.
Eurostat provides the metadata on the aggregated data in different forms like CSV files.\tb{some metadata in form of SDMX and data in form of CSV files?}
...
EXAMPLE
\end{ex}
\tb{Achim, can we start with the PHDD representation?}
