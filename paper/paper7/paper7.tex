...
OWL 2 \emph{hasKey} implements \emph{keyfor} and can therefore be used to identify resources uniquely, to merge resources with identical key property values, and to recognize constraint violations.
...
\textbf{Coverage.}
In order to express the temporal coverage of series, studies, or data sets dates can be stated.
...
We used $\mathcal{C}_{C}$ and $\mathcal{C}_{T}$ constraints 
to evaluate the metadata and data quality of existing large real world aggregated and person-level data sets.
We provide a triple store\footnote{\textcolor{red}{\url{http://multiweb.gesis.org/openrdf-workbench/}}}, containing these data sets, 
and the constraints\footnote{\url{https://github.com/boschthomas/rdf-validation/constraints}} in form of SPARQL CONSTRUCT queries.
We validated 
(1) \emph{QB} data sets (\textcolor{red}{XX}),
(2) \emph{Disco} data sets (overall \textcolor{red}{XX} resources exported from \emph{Missy} - 5 series, \textcolor{red}{XX} studies, \textcolor{red}{XX} data sets, \textcolor{red}{XX} theoretical concepts, \textcolor{red}{XX} variables, \textcolor{red}{XX} questions, \textcolor{red}{XX} codes, \textcolor{red}{XX} category statistics (frequencies), and \textcolor{red}{XX} summary statistics),
(3) \emph{DCAT} data sets (\textcolor{red}{XX}),
(4) \emph{PHDD} data sets (\textcolor{red}{XX}),
(5) \emph{SKOS} data sets (The Thesaurus for the Social Sciences (TheSoz), the STW Thesaurus for Economics of the ZBW, and the AGROVOC thesaurus of FAO), and
(6) \emph{XKOS} data sets (\textcolor{red}{XX}).

The correctness of $\mathcal{C}_{C}$ and $\mathcal{C}_{T}$ constraints, i.e., the gold standard has been designed by a SBE domain expert.
As proof-of-concept, we use the \emph{RDF Validator} to offer examples for both valid and invalid RDF data regarding particular constraints. 
