...
We classified both RDF constraint types and RDF constraints to evaluate the quality of metadata and data which may be represented by any vocabulary.
...
\emph{Simple constraint types} is the set of constraint types whose constraints can be easily defined without much effort in addition to \emph{vocabulary constraints}.
...
\emph{Complex constraints} show the importance to develop constraint languages enabling to describe more complex constraints. 
...
According to the constraints' default severity level, the complete set of constraints encompasses three disjoint \emph{sets of constraints}:
\begin{itemize}
...
The difference between \emph{warning} and \emph{error constraints} constraints is that data, not conforming to \emph{warning constraints} but conforming to \emph{error constraints}, could be processed further,
whereas data, not corresponding to \emph{error constraints}, cannot be processed further after validation. 
As constraints of \emph{vocabulary constraint types} are derived from explicitly stated semantics of vocabularies, 
their default severity levels are in most cases very strong (\emph{error}) and 
in average stronger than the severity levels of constraints assigned to \emph{simple and complex constraint types}.
As a consequence, violating many constraints of \emph{vocabulary constraint types} is an indicator for bad (meta)data quality\footnote{For simplicity reasons, we only assign severity levels to \emph{vocabulary constraints} in this paper in case they differ from \emph{error constraints}.}.
...
of 53 different types on six vocabularies (\emph{Disco}, \emph{QB}, \emph{SKOS}, \emph{PHDD}, \emph{DCAT}, \emph{XKOS}) and classifies them according to the complexity level of their type and their severity level. 
For 114 of these constraints, we evaluate the data quality of 15,694 data sets (4,26 billion triples) of \emph{SBE} research data on three vocabularies (\emph{Disco}, \emph{QB}, \emph{SKOS}) obtained from 33 SPARQL endpoints.
These vocabularies are the central and most representative ones within the \emph{SBE} community. 
We distinct two classes of vocabularies: 
(1) established vocabularies which are widely adopted and accepted and 
(2) newly developed vocabularies which are either recently published or are still in the publication process.
...
We investigate the quality of data conforming to the established vocabularies \emph{QB} and \emph{SKOS} and to the newly developed vocabulary \emph{Disco} which will be published in 2015. 

We evaluated the metadata quality of large real world aggregated (\emph{QB}), person-level (\emph{Disco}), and thesauri (\emph{SKOS}) data sets by means of both $\mathcal{C}_{C}$ and $\mathcal{C}_{T}$ constraints of the majority of the constraint types.
...
The constraints and their resulting constraint violations are grouped by vocabulary, complexity level of their type, and severity level.
The number of evaluated triples and data sets differs between the vocabularies
as we evaluated 3.8 billion QB, 480 million SKOS, and 10 million Disco triples.
In order to get suitable statements for all vocabularies,  
we normalize the total values which apply for all vocabularies
...
Almost 1/2 of all 213 constraints and more than 50\% of the \emph{Disco} and the \emph{QB} constraints are vocabulary constraints.
The \emph{SKOS} constraints, on the other side, are equally distributed among the three disjoint sets of constraint types. 
\begin{hyp}
A significant amount of 46 \% of the constraints are directly extractable from established and newly defined vocabularies. 
\end{hyp} 

Nearly 1/2 of the constraint violations are caused by complex constraints, 1/3 by simple constraints, and 1/5 by vocabulary constraints.
2/3 of the constraint violations, thrown for \emph{Disco} data sets, result from vocabulary constraints,
almost only conplex constraints raised constraint violations for \emph{QB} data sets, and 
nearly 80\% of the \emph{SKOS} constraint violations are caused by simple constraints.

\begin{hyp}
The fact that only 1/5 of all constraint violations result from vocabulary constraints, 
even though, 46\% of all constraints are vocabulary constraints,
indicates that in general data conforms to the intended syntax and semantics of vocabularies.
\end{hyp}

For established vocabularies, vocabulary constraints caused nearly 0\% of all constraint violations (e.g. only 1.777 constraint violations for \emph{QB} constraints).  
For newly defined vocabularies, however, 2/3 of all constraint violations are raised by vocabulary constraints. 
\begin{hyp}
Complex constraints are met by data represented by established vocabularies but not by data represented by newly developed vocabularies. 
\end{hyp}
It is likely that a newly developed vocabulary is still subject of constant change, that published data may not be consistent with its current version,
and that early adopters did not properly understand its formal specification.
...
\end{hyp} 
\emph{Vocabulary} and \emph{simple constraints} can be easily, concisely, and intuitively expressed by either modeling languages (e.g., \emph{RDF}, \emph{RDFS}, \emph{OWL 2}) or constraint languages (e.g., \emph{ShEx}, \emph{ReSh}, \emph{SPIN}). 
This is not the case, however, for \emph{complex constraints} which in most cases still have to be expressed by plain \emph{SPARQL}. 
As almost the half of all constraint violations are caused by \emph{complex constraints}, 
data quality can be significantly improved when existing constraint languages are extended or new constraint languages are developed to express \emph{complex constraints} more easily, concisely, and intuitively.
...
\tb{it has to be ensured that constraints within a particular set are not violated randomly  
in case there are only a few constraints within a set} 

\tb{established vocabularies meet vocabulary constraints / data sets represented by established vocabularies meet vocabulary constraints which indicates good data quality}

\tb{QB data sets contain data and metadata and therefore there are the differences between constraint violations}
